...
const VERSION='10.5.4-beta.56-runtime-snapshot-main-v2-offline-test';
...
async function refresh(options={}){
  ...
  try{
    if(options.simulate_offline===true){const e=new Error('SH-0007 controlled offline acceptance simulation');e.code='ENETDOWN';throw e}
    snap=await requestSnapshot(gate,options.timeout_ms)
  }catch(error){if(!offlineEligible(error))return {ok:false,version:VERSION,mode:'fail-closed',reason_code:String(error?.code||'SNAPSHOT_ONLINE_REJECTED'),error:String(error?.message||error)};try{const cached=s.loadOffline(expected);return {ok:true,version:VERSION,mode:'offline-cache',sequence:cached.sequence,snapshot_id:text(cached.snapshot.snapshot_id),expires_at:cached.snapshot.expires_at,baseline_version:cached.snapshot.baseline_version,policy_version:cached.snapshot.policy_version,runtime_environment:cached.snapshot.runtime_environment,composition_version:cached.snapshot.composition_version,decision_count:Object.keys(cached.snapshot.decisions||{}).length,network_error:String(error?.code||error?.message||error),simulated_offline:options.simulate_offline===true}}catch(cacheError){return {ok:false,version:VERSION,mode:'fail-closed',reason_code:String(cacheError?.code||error?.code||'SNAPSHOT_UNAVAILABLE'),error:String(cacheError?.message||error?.message||cacheError||error),network_error:String(error?.code||error?.message||error)}}}
