...
const VERSION='10.5.4-beta.56-runtime-snapshot-main-v1';
const CLOUD_HOST='ikppryeavoabnugcijeq.supabase.co';
const SNAPSHOT_PATH='/functions/v1/runtime-access-snapshot-v1';
...
function requestSnapshot(gate,timeoutMs=12000){
  return new Promise((resolve,reject)=>{
    ...
    const req=https.request({hostname:CLOUD_HOST,path:SNAPSHOT_PATH,method:'POST',headers:{'Content-Type':'application/json','Content-Length':Buffer.byteLength(body),'apikey':CLOUD_PUBLISHABLE_KEY}},res=>{
...
function decisionFrom(snapshot,featureCode){
  const code=text(featureCode);
  if(!code)return {allowed:false,reason_code:'FEATURE_CODE_REQUIRED'};
  const d=snapshot?.decisions?.[code];
  if(!d||typeof d!=='object')return {allowed:false,reason_code:'FEATURE_UNKNOWN'};
  return {allowed:d.allowed===true,reason_code:text(d.reason_code)||'DENIED',feature_code:code,readiness_status:d.readiness_status||null,blocked_by:d.blocked_by||null,dependency_reason:d.dependency_reason||null};
}
...
function state(){
  const gate=sandbox(),s=store();
  if(!gate.ok)return {ok:false,version:VERSION,mode:'disabled',sandbox:{ok:false,reasons:gate.reasons},high_water:s.highWater()};
  try{const cached=s.loadOffline(expectedFrom(gate));return {ok:true,version:VERSION,mode:'safe-cache',sequence:cached.sequence,high_water:s.highWater(),snapshot_id:text(cached.snapshot.snapshot_id),expires_at:cached.snapshot.expires_at,baseline_version:cached.snapshot.baseline_version,policy_version:cached.snapshot.policy_version,runtime_environment:cached.snapshot.runtime_environment,sandbox:{ok:true,support_code:BETA_SUPPORT,business_id:BETA_BUSINESS_ID}}}catch(e){return {ok:false,version:VERSION,mode:'fail-closed',reason_code:String(e.code||'SAFE_SNAPSHOT_MISSING'),high_water:s.highWater(),sandbox:{ok:true,support_code:BETA_SUPPORT,business_id:BETA_BUSINESS_ID}}}
}
function feature(featureCode){
  const gate=sandbox();if(!gate.ok)return {allowed:false,reason_code:'RUNTIME_SNAPSHOT_SANDBOX_LOCK'};
  try{const cached=store().loadOffline(expectedFrom(gate));return decisionFrom(cached.snapshot,featureCode)}catch(e){return {allowed:false,reason_code:String(e.code||'SNAPSHOT_UNAVAILABLE'),feature_code:text(featureCode)}}
}
function installRuntimeSnapshotMain(){
  if(installed)return module.exports.api;installed=true;
  ipcMain.handle('runtime-snapshot:refresh',(_e,input)=>refresh(input||{}));
  ipcMain.handle('runtime-snapshot:state',()=>state());
  ipcMain.handle('runtime-snapshot:feature',(_e,code)=>feature(code));
