...
function retail(){try{return global.SharawlaRuntimeCore?.hasEngine?.('retail')&&typeof global.runtimeAllPages==='function'&&global.runtimeAllPages().includes('marketSettings')}catch{return false}}
...
async function enhanceCheckout(){
 ...
 head.dataset.fullRetail='1';select.classList.remove('hidden');
 select.innerHTML='<option value="takeaway">بيع تجزئة / استلام فوري</option><option value="pickup">استلام من الفرع</option><option value="delivery">دليفري</option>';
 const b=branch();
 const [zones,drivers]=await Promise.all([
   ...
   rows('drivers',`select=id,name,phone,branch_id,active&active=eq.true${b?`&branch_id=eq.${b}`:''}&order=name`)
 ]);
 const box=document.createElement('div');box.id='deliveryFields';box.className='delivery-fields hidden';
...
function observe(){if(wiring)return;wiring=true;const o=new MutationObserver(()=>enhanceCheckout());o.observe(document.body,{childList:true,subtree:true});enhanceCheckout()}
...
function start(){if(retail())observe()}
