...
async function openCenter(){
  ...
  m.onclick=async e=>{
    ...
    if(e.target.closest('[data-v2-reset-test]')){const b=e.target.closest('[data-v2-reset-test]');if(!confirm('سيتم حذف العمليات غير المتزامنة التجريبية على SH-0007 / TEST فقط. متابعة؟'))return;b.disabled=true;try{const license=await global.topBurgerDesktop?.licenseState?.get?.();const branchName=text(typeof state!=='undefined'&&state?.branch?.name||typeof currentBranchName==='function'?currentBranchName():'TEST')||'TEST';await api()?.resetTestQueue?.({support_code:text(license?.support_code),branch_name:branchName});if(typeof setOfflineQueue==='function')await setOfflineQueue([]);try{const rows=await global.topBurgerDesktop?.operations?.list?.('pending')||[];for(const r of rows)await global.topBurgerDesktop?.operations?.status?.(r.client_tx_id,'discarded','SH-0007 TEST RESET')}catch{}toast('تم تصفير بيانات الاختبار')}catch(err){toast(err.message)}m.remove();await openCenter();await refresh();return}
    if(e.target.closest('[data-v2-sync]')){e.target.closest('[data-v2-sync]').disabled=true;try{const result=await global.SharawlaOfflineV2Transport?.syncNow?.();const d=result?.scope_diagnostics?.after||result?.scope_diagnostics?.before;if(result?.attempted===0&&Number(d?.outside_employee_scope_count||0)>0){const groups=(d.outside_employee_scope||[]).map(x=>`Employee ${x.employee_id}: ${x.count} (${x.status}) Seq ${x.first_sequence}-${x.last_sequence}`).join(' | ');try{toast(`لا توجد حركة قابلة للمزامنة للموظف الحالي ${d.current_employee_id}. خارج النطاق: ${d.outside_employee_scope_count}. ${groups}`)}catch{}}else if(result?.attempted===0){const rows=d?.current_scope_rows||[];const blocked=rows.filter(x=>x.dependency_synced===false);const delayed=rows.filter(x=>x.retry_due===false);const detail=blocked.length?` • Dependency غير متزامنة: Seq ${blocked.map(x=>x.device_sequence).join(',')}`:delayed.length?` • Retry غير مستحق بعد: Seq ${delayed.map(x=>x.device_sequence).join(',')}`:rows.length?` • الحركة داخل النطاق ومستحقة لكن لم يتم Claim: Seq ${rows.map(x=>x.device_sequence).join(',')}`:'';try{toast(`لم يتم Claim لأي حركة. Employee ${d?.current_employee_id||'—'} • Current scope ${d?.current_scope_count||0}${detail}`)}catch{}}}catch(err){try{toast(err.message)}catch{}}m.remove();await openCenter();await refresh();return}
