...
async function dbSet(k,v){try{if(typeof odbSet==='function')return await odbSet(k,v)}catch{}try{return await global.odbSet?.(k,v)}catch{return v}}
...
async function baselineRows(table,query){
 ...
 if(table==='products')rows=st.products||[];
   ...
   rows=[];const q=await getQueue();rows.push(...q.filter(x=>x.type==='expense'&&x.local_expense).map(x=>x.local_expense));
 }
   ...
   const q=await getQueue();rows=q.filter(x=>x.type==='return').flatMap(x=>x.local_items||[]);
 }
 else if(table==='return_payments'){
   const q=await getQueue();rows=q.filter(x=>x.type==='return').flatMap(x=>x.local_payments||[]);
 }
...
async function restRecovery(table,query='',opt={}){
 ...
 if(onlineAuthorized()){
   try{const rows=await baseRest(table,query,opt);await dbSet(readKey(table,query),rows);return rows}catch(e){if(!netError(e))throw e}
 }
 const exact=await dbGet(readKey(table,query));if(Array.isArray(exact))return clone(exact);
 const local=await baselineRows(table,query);if(local!==null)return local;
...
async function reconcileOpenShiftBeforeSync(){
 ...
 let q=await getQueue();const redundant=q.filter(j=>j.type==='shift_open'&&Number(j.local_shift?.branch_id||j.p_branch_id||0)===branchId()&&Number(j.local_shift?.employee_id||j._scope?.employee_id||employeeId())===employeeId());
...
async function saveOfflineSaleRecovery(orderPayload,itemPayload,payRows,providedClientTx=null){
 const wasOffline=!navigator.onLine;const out=await baseSaveOfflineSale(orderPayload,itemPayload,payRows,providedClientTx);
 if(!wasOffline||!out?.order)return out;
 ...
 try{const q=await getQueue();const j=q.find(x=>String(x.client_tx_id)===String(out.client_tx_id||providedClientTx));if(j?.local_order){j.local_order.invoice_number=`OFF-${n}`;j.local_order.bon_number=`OFF-${n}`;await setQueue(q);try{await global.topBurgerDesktop?.operations?.put?.(j)}catch{}}}catch{}
 return out;
}
...
async function updateNextBonRecovery(){
 ...
 const n=await nextOfflineBon(sh.id,false);el.innerHTML=`<span>رقم البون التالي</span><b>OFF-${n}</b><small>أوفلاين • وردية ${text(sh.id).slice(0,12)}</small>`;
}
...
async function warmRuntimeCaches(){
 ...
 for(const [t,q] of calls){try{const rows=await baseRest(t,q);await dbSet(readKey(t,q),rows);if(t==='shifts')await dbSet(`shiftHistory:${b}`,rows)}catch{}}
...
function install(){
 ...
 installed=true;
 global.__SharawlaBeta554RuntimeRecovery=Object.freeze({version:VERSION,installed:true,warmRuntimeCaches,reconcileOpenShiftBeforeSync,syncNow:syncRecovery,offlineReadFallback:true,openShiftContinuity:true,reconnectReconcile:true,bonSequencing:true});
