...
async function persistQueue47(q){
  await setQueue47(q);
...
async function markDone47(job){
  const q=(await getQueue47()).filter(x=>x.client_tx_id!==job.client_tx_id);await setQueue47(q);
...
async function currentOpenShift47(job){
  const branch=Number(job.p_branch_id||job.local_shift?.branch_id||0),employee=Number(job.local_shift?.employee_id||job._scope?.employee_id||global.state?.employee?.id||0);if(!branch||!employee)return null;
  try{const rows=await global.rest('shifts',`select=*&branch_id=eq.${branch}&employee_id=eq.${employee}&status=eq.open&closed_at=is.null&order=opened_at.desc&limit=2`);return Array.isArray(rows)&&rows.length===1?rows[0]:null}catch{return null}
}
...
async function syncOne47(job){
  if(job.type==='shift_open'){
    try{
      const sh=await global.rpc('open_pos_shift_idempotent',{p_branch_id:job.p_branch_id,p_opening_cash:job.p_opening_cash,p_client_tx_id:job.client_tx_id});
      await remapShift47(job.local_shift_id,sh.id);if(typeof global.rememberOpenShift==='function')await global.rememberOpenShift(sh);await markDone47(job);return 'done';
    }catch(e){if(alreadyOpen47(e)){const sh=await currentOpenShift47(job);if(sh?.id){await remapShift47(job.local_shift_id,sh.id);if(typeof global.rememberOpenShift==='function')await global.rememberOpenShift(sh);await markDone47(job);return 'reconciled'}}throw e}
  }
  const q=await getQueue47();if(dependsOnShift47(job,q))return 'blocked';
  if(job.type==='sale')await global.rpc(job.engine==='retail'?'create_retail_pos_order_atomic':'create_pos_order_atomic',{p_order:job.p_order,p_items:job.p_items,p_payments:job.p_payments});
  else if(job.type==='expense')await global.rpc('create_pos_expense_idempotent',{p_shift_id:Number(job.p_shift_id),p_description:job.p_description,p_amount:job.p_amount,p_client_tx_id:job.client_tx_id});
  else if(job.type==='return')await global.rpc(job.engine==='retail'?'create_retail_order_return_idempotent':'create_order_return_idempotent',{p_order_id:job.p_order_id,p_reason:job.p_reason,p_notes:job.p_notes,p_items:job.p_items,p_payments:job.p_payments,p_client_tx_id:job.client_tx_id});
  else if(job.type==='shift_close')await global.rpc('close_pos_shift_idempotent',{p_shift_id:Number(job.p_shift_id),p_closing_cash:job.p_closing_cash,p_metrics:job.p_metrics,p_client_tx_id:job.client_tx_id});
...
async function syncOfflineQueue47(){
  if(syncBusy47||!navigator.onLine||!global.session?.access_token)return;
  syncBusy47=true;let done=0,reconciled=0,failed=0,blocked=0,conflicts=0;
  try{
    try{if(typeof global.refreshSessionIfNeeded==='function')await global.refreshSessionIfNeeded()}catch(e){console.warn('Beta47 session refresh before sync',e);return}
    ...
    for(const job of snapshot){
      ...
      try{
        const r=await syncOne47(current);
        if(r==='done')done++;else if(r==='reconciled')reconciled++;else if(r==='blocked'){blocked++;await mark47(current,'blocked',new Error('الحركة تنتظر مزامنة الوردية المرتبطة بها'))}
    ...
    try{if(typeof global.refreshPendingSyncBadge==='function')await global.refreshPendingSyncBadge()}catch{}
