...
async function renderInventory(){
 if(isRetailProfile())return renderRetailInventory();
 if(!state.settings.enable_inventory){$('#page').innerHTML='<div class="empty">المخزون غير مفعّل من الإعدادات</div>';return;}
 const rows=await rest('ingredient_stock',`select=id,branch_id,quantity,ingredients(name,unit)&branch_id=eq.${currentBranchId()}&order=id`);
 $('#page').innerHTML=`<div class="panel"><h2>مخزون الخامات</h2><div class="table-wrap"><table><thead><tr><th>الخامة</th><th>الوحدة</th><th>الفرع</th><th>الرصيد</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${esc(r.ingredients?.name||'')}</td><td>${esc(r.ingredients?.unit||'')}</td><td>${branchName(r.branch_id)}</td><td>${r.quantity}</td></tr>`).join('')}</tbody></table></div></div>`
}
