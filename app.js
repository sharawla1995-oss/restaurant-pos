...
async function rc1RequireCloudOnline(action='العملية'){
 ...
 if(!navigator.onLine)throw internetError();
 if(!cfg?.url||!cfg?.key||!session?.access_token){const e=new Error(`${label}: جلسة الاتصال غير جاهزة. لم يتم تغيير أي بيانات.`);e.code='ONLINE_ONLY_AUTH_REQUIRED';throw e}
 const controller=typeof AbortController==='function'?new AbortController():null;const timer=controller?setTimeout(()=>controller.abort(),5000):null;
 try{
  const r=await fetch(`${cfg.url}/auth/v1/user`,{method:'GET',cache:'no-store',headers:{apikey:cfg.key,Authorization:`Bearer ${session.access_token}`},...(controller?{signal:controller.signal}:{})});
  if(r.status===401||r.status===403){const e=new Error(`${label}: انتهت الجلسة أو لا توجد صلاحية. لم يتم تغيير أي بيانات.`);e.code='ONLINE_ONLY_AUTH_REQUIRED';throw e}
  if(!r.ok)throw new Error(`تعذر التحقق من اتصال الخادم (${r.status})`);
  return true;
 }catch(e){if(e?.code==='ONLINE_ONLY_AUTH_REQUIRED'||e?.code==='ONLINE_ONLY_INTERNET_REQUIRED')throw e;if(e?.name==='AbortError'||isNetError(e))throw internetError(e);throw e}
 finally{if(timer)clearTimeout(timer)}
}
window.__SharawlaRC1OnlineOnlyGuard=Object.freeze({version:'rc1-online-only-v1',requireCloud:rc1RequireCloudOnline});
...
function point4UuidV4(){const provider=window.__SharawlaPoint4UuidV4;if(!provider||typeof provider.next!=='function'){const e=new Error('Point 4 canonical UUIDv4 provider is unavailable');e.code='POINT4_IDENTITY_UUID_V4_REQUIRED';throw e}return provider.next()}
function point4EffectLineKey(effect,lineUid){return `v1:stock:${effect}:${lineUid}`}
function point4SaleIdentity(clientTx,itemPayload){const documentUid=point4UuidV4(),sourceDocumentId=`uuid:${documentUid}`;return{contract:POINT4_IDENTITY_V1,client_tx_id:clientTx,document_uid:documentUid,source_document_type:'sale',source_document_id:sourceDocumentId,lines:itemPayload.map(x=>{const line_uid=point4UuidV4();return{...x,line_uid,effect_line_key:point4EffectLineKey('sale',line_uid)}})}}
function point4ReturnIdentity(clientTx,order,selected){const documentUid=point4UuidV4(),sourceDocumentId=`uuid:${documentUid}`,originalSourceDocumentId=order?.source_document_id||(order?.document_uid?`uuid:${String(order.document_uid).toLowerCase()}`:(Number(order?.id)>0?`db:${Number(order.id)}`:null));return{contract:POINT4_IDENTITY_V1,client_tx_id:clientTx,document_uid:documentUid,source_document_type:'sale_return',source_document_id:sourceDocumentId,original_source_document_id:originalSourceDocumentId,lines:selected.map(x=>{const line_uid=x.line_uid||point4UuidV4();return{...x,line_uid,effect_line_key:x.effect_line_key||point4EffectLineKey('sale_return',line_uid),source_document_id:sourceDocumentId,original_source_document_id:originalSourceDocumentId}})}}
async function point4RememberOrderIdentity(order,identity){if(!order?.id||!identity?.document_uid)return order;const value={document_uid:identity.document_uid,source_document_id:identity.source_document_id,point4_identity_contract:identity.contract};await odbSet(`point4OrderIdentity:${order.id}`,value);return Object.assign(order,value)}
async function point4HydrateOrderIdentity(order){if(!order?.id||order.document_uid)return order;let value=await odbGet(`point4OrderIdentity:${order.id}`);if(!value&&window.topBurgerDesktop?.offlineV2?.outbox)try{const rows=await window.topBurgerDesktop.offlineV2.outbox('synced');const row=(rows||[]).find(x=>x.operation_type==='sale'&&String(x.server_ack?.result?.order?.id||x.server_ack?.server_entity_id||'')===String(order.id)),p=row?.envelope?.payload?.rpc_payload?.p_order;if(p?.document_uid&&p?.source_document_id)value={document_uid:p.document_uid,source_document_id:p.source_document_id,point4_identity_contract:p.point4_identity_contract}}catch{}if(value){await odbSet(`point4OrderIdentity:${order.id}`,value);Object.assign(order,value)}return order}
...
async function saveOfflineSale(orderPayload,itemPayload,payRows,providedClientTx=null){assertOfflineCommerceProfile('sale');const q=await offlineQueue(),n=offlineOrderNo(),clientTx=providedClientTx||uuid(),created=new Date().toISOString();setOfflineOrderNo(n);const localId=`offline-${clientTx}`,offlineReference=`OFF-${String(n).padStart(4,'0')}-${String(clientTx).replace(/-/g,'').slice(0,6).toUpperCase()}`;const localOrder={...orderPayload,id:localId,client_tx_id:clientTx,invoice_number:null,bon_number:null,offline_reference:offlineReference,_official_number_pending:true,created_at:created,payment_status:'confirmed',_offline:true};const localItems=itemPayload.map(x=>({...x,id:`${localId}-line-${x.line_uid}`,order_id:localId}));const job={type:'sale',engine:isRetailProfile()?'retail':'restaurant',client_tx_id:clientTx,created_at:created,p_order:{...orderPayload,client_tx_id:clientTx},p_items:itemPayload,p_payments:payRows,local_order:localOrder,local_items:localItems};q.push(job);await setOfflineQueue(q);try{if(window.topBurgerDesktop?.operations?.put)await window.topBurgerDesktop.operations.put(job)}catch{}return {order:localOrder,items:localItems,client_tx_id:clientTx}}
...
const PAGE_RENDERERS=Object.freeze({
  ...
  deliveryOrders:(...args)=>window.renderDeliveryOrders(...args),
...
function officialNumberPending(o){return o?._official_number_pending===true||(!Number(o?.bon_number)&&!!o?.offline_reference)}
function localOrderReference(o){return String(o?.offline_reference||'').trim()}
function bonDisplay(o){if(Number(o?.bon_number)>0)return String(o.bon_number);const ref=localOrderReference(o);return ref||(o?.order_number||'#'+o?.id);}
async function updateNextBonBadge(){const el=$('#nextBonBadge');if(!el)return;try{const sh=await getOpenShift();if(!sh){el.innerHTML='<span>رقم البون التالي</span><b>—</b><small>افتح وردية</small>';return}if(!isServerShiftId(sh.id)){const q=await offlineQueue(),pending=q.filter(j=>j.type==='sale'&&String(j.local_order?.shift_id??j.p_order?.shift_id??'')===String(sh.id)).length;el.innerHTML=`<span>رقم البون التالي</span><b>بعد المزامنة</b><small>${pending?`${pending} بون محلي بانتظار الرقم الرسمي`:'الرقم الرسمي يخصصه السيرفر'} • وردية محلية</small>`;return}const r=await rest('shift_bon_counters',`select=next_number&shift_id=eq.${sh.id}&limit=1`).catch(()=>[]);let next=Number(r?.[0]?.next_number||0);if(!next){const oo=await rest('orders',`select=bon_number&shift_id=eq.${sh.id}&order=bon_number.desc&limit=1`).catch(()=>[]);next=Number(oo?.[0]?.bon_number||0)+1}el.innerHTML=`<span>رقم البون التالي</span><b>${next||1}</b><small>وردية #${sh.id}</small>`}catch(e){el.innerHTML='<span>رقم البون التالي</span><b>—</b>'}}
...
function assertOfflineCommerceProfile(operation='commerce'){const profile=currentPosProfile();if(profile==='restaurant'||profile==='retail')return profile;const e=new Error(`Offline ${operation} غير مدعوم للنشاط الحالي (${profile||'غير محدد'})`);e.code='OFFLINE_COMMERCE_PROFILE_UNSUPPORTED';e.profile=profile;e.operation=operation;throw e}
...
function retailQtyStep(productId){const x=retailProductSetting(productId);return Math.max(.001,Number(x.allow_decimal?x.qty_step:1)||1)}
function retailQtyNormalize(productId,qty){const st=retailProductSetting(productId),step=retailQtyStep(productId);let q=Number(qty||0);q=Math.round(q/step)*step;if(!st.allow_decimal)q=Math.round(q);return Math.max(0,Math.round(q*1000)/1000)}
function retailQtyLabel(productId,qty){const st=retailProductSetting(productId);return `${Number(qty||0).toFixed(st.allow_decimal?3:0)} ${({piece:'قطعة',kg:'كجم',g:'جم',liter:'لتر',ml:'مل'})[st.unit_type]||st.unit_type||''}`.trim()}
async function loadRetailMarketData(force=false){if(!isRetailProfile())return;if(!force&&retailMarket.loadedBranch===Number(currentBranchId()))return;let ps=[],offers=[],ops=[];try{[ps,offers,ops]=await Promise.all([rest('retail_product_settings','select=*'),rest('retail_offers',`select=*&active=eq.true&or=(branch_id.is.null,branch_id.eq.${Number(currentBranchId())})&order=priority,id`),rest('retail_offer_products','select=*')])}catch(e){console.warn('retail market beta17 data',e);ps=[];offers=[];ops=[]}retailMarket.productSettings=new Map(ps.map(x=>[String(x.product_id),x]));retailMarket.offers=offers||[];retailMarket.offerProducts=ops||[];retailMarket.loadedBranch=Number(currentBranchId())}
function retailActiveOfferFor(productId){const now=Date.now();return retailMarket.offers.find(o=>{if(o.starts_at&&new Date(o.starts_at).getTime()>now)return false;if(o.ends_at&&new Date(o.ends_at).getTime()<now)return false;const ids=retailMarket.offerProducts.filter(x=>String(x.offer_id)===String(o.id)).map(x=>String(x.product_id));return !ids.length||ids.includes(String(productId))})||null}
function retailOfferDiscount(){if(!isRetailProfile())return 0;let total=0;for(const i of state.cart){const o=retailActiveOfferFor(i.product_id);if(!o)continue;const qty=Number(i.qty||0),price=Number(i.price||0);if(o.rule_type==='percent')total+=price*qty*Math.max(0,Math.min(100,Number(o.value||0)))/100;else if(o.rule_type==='fixed')total+=Math.min(price*qty,Math.max(0,Number(o.value||0))*qty);else if(o.rule_type==='second_half')total+=Math.floor(qty/2)*price*.5;else if(o.rule_type==='buy_x_get_y'){const buy=Math.max(.001,Number(o.buy_qty||1)),get=Math.max(.001,Number(o.get_qty||1)),sets=Math.floor(qty/(buy+get));total+=sets*get*price}}return Math.round(total*100)/100}
function decodeRetailEmbeddedBarcode(code){const raw=retailBarcodeValue(code);if(!/^\d{13}$/.test(raw))return null;const prefix=raw.slice(0,2),plu=raw.slice(2,7),embedded=Number(raw.slice(7,12));for(const [pid,st] of retailMarket.productSettings){if(!['weight','price'].includes(st.barcode_mode))continue;if(st.weight_prefix&&String(st.weight_prefix)!==prefix)continue;if(String(st.plu_code||'').padStart(5,'0')!==plu)continue;const p=state.products.find(x=>String(x.id)===pid);if(!p||p.active===false||!productAvailableAtBranch(p))continue;const divisor=Math.max(.001,Number(st.embedded_divisor||1000));let qty=embedded/divisor;if(st.barcode_mode==='price'){const unit=Number(effectiveProductPrice(p)||0);if(unit<=0)return null;qty=(embedded/divisor)/unit}qty=retailQtyNormalize(p.id,qty);return qty>0?{product:p,qty,mode:st.barcode_mode}:null}return null}
function addRetailProductToCart(p,forcedQty=null){
 if(!p)return;
 const price=effectiveProductPrice(p),step=retailQtyStep(p.id),qty=retailQtyNormalize(p.id,forcedQty===null?step:forcedQty);
 ...
 if(existing){existing.qty=retailQtyNormalize(p.id,Number(existing.qty||0)+qty);drawCart();return}
 state.cart.push({product_id:p.id,name:p.name,price:Number(price),base_price:Number(price),cost:Number(p.cost||0),qty:qty||step,modifiers:[],removed:[],notes:''});
 drawCart();
}
...
async function renderRetailPOS(){
 ...
 $('#page').innerHTML=`<div class="retail-checkout-shell">
  ...
  <aside class="cart retail-cart">
   ...
   <div class="cart-head retail-sale-head">
    <div class="retail-cart-tools"><button id="holdRetailSale" class="secondary" type="button">⏸️ تعليق</button><button id="resumeRetailSale" class="secondary" type="button">▶️ استرجاع</button></div>
   ...
   <div class="cart-foot">
    ...
    <div class="totline" id="retailOfferLine"><span>عروض الماركت</span><b id="retailOfferAmount">0</b></div>
 ...
 if(scan){scan.addEventListener('keydown',e=>{if(e.key!=='Enter')return;e.preventDefault();const code=retailBarcodeValue(scan.value);if(!code)return;const p=findRetailProductByBarcode(code);if(p){addRetailProductToCart(p);scan.value='';scan.focus();return}const embedded=decodeRetailEmbeddedBarcode(code);if(!embedded){toast(`باركود غير موجود: ${code}`);scan.select();return}addRetailProductToCart(embedded.product,embedded.qty);toast(`${embedded.mode==='weight'?'وزن':'قيمة'}: ${retailQtyLabel(embedded.product.id,embedded.qty)}`);scan.value='';scan.focus()});}
...
function cartCalc(){const subtotal=state.cart.reduce((s,i)=>s+(Number(i.price||0)*Number(i.qty||0)),0);const f=financialCfg();let discountValue=Number($('#discount')?.value||0),discountType=$('#discountType')?.value||(f.discount_mode==='percent'?'percent':'amount');if(f.discount_mode==='amount')discountType='amount';if(f.discount_mode==='percent')discountType='percent';if(!f.discount_enabled||!discountAllowed())discountValue=0;if(discountType==='percent')discountValue=Math.min(Math.max(0,discountValue),Number(f.max_discount_percent||100));let manualDiscount=discountType==='percent'?subtotal*discountValue/100:Math.max(0,discountValue);manualDiscount=Math.min(subtotal,manualDiscount,subtotal*Number(f.max_discount_percent||100)/100);const promoDiscount=Math.max(0,Math.min(Number(state.activePromo?.discount||0),Math.max(0,subtotal-manualDiscount)));const retailOfferDiscountValue=Math.max(0,Math.min(retailOfferDiscount(),Math.max(0,subtotal-manualDiscount-promoDiscount)));const discount=Math.min(subtotal,manualDiscount+promoDiscount+retailOfferDiscountValue);const base=Math.max(0,subtotal-discount);let taxAmount=f.tax_enabled?base*Number(f.tax_rate||0)/100:0;if(f.tax_enabled&&f.prices_include_tax)taxAmount=base-(base/(1+Number(f.tax_rate||0)/100));const serviceAmount=f.service_enabled?base*Number(f.service_rate||0)/100:0;let deliveryFee=0;if($('#orderType')?.value==='delivery')deliveryFee=Math.max(0,Number($('#manualDeliveryFee')?.value||0)||0);const total=Math.max(0,base+(f.tax_enabled&&!f.prices_include_tax?taxAmount:0)+serviceAmount+deliveryFee);return{subtotal,discount,manualDiscount,promoDiscount,retailOfferDiscount:retailOfferDiscountValue,discountType,discountValue,taxAmount,serviceAmount,deliveryFee,total}}
function drawCart(){if(!$('#cartItems'))return;$('#cartItems').innerHTML=state.cart.length?state.cart.map((i,n)=>`<div class="cart-item"><div class="cart-row"><b>${esc(i.name)}</b><b>${money(i.price*i.qty)}</b></div>${i.modifiers?.length?`<small>+ ${i.modifiers.map(x=>esc(x.name)).join('، ')}</small>`:''}${i.removed?.length?`<small>بدون: ${i.removed.map(esc).join('، ')}</small>`:''}${i.notes?`<small>ملاحظة: ${esc(i.notes)}</small>`:''}<div class="qty"><button data-a="plus" data-i="${n}">+</button><b>${isRetailProfile()?retailQtyLabel(i.product_id,i.qty):i.qty}</b>${isRetailProfile()?`<button data-a="editqty" data-i="${n}">⚖️</button>`:''}<button data-a="minus" data-i="${n}">−</button><button data-a="del" data-i="${n}">🗑</button></div></div>`).join(''):'<div class="empty">أضف أصناف للأوردر</div>';$('#cartItems').onclick=e=>{const b=e.target.closest('button[data-a]');if(!b)return;const i=state.cart[+b.dataset.i];invalidateActivePromo();if(b.dataset.a==='plus')i.qty=isRetailProfile()?retailQtyNormalize(i.product_id,Number(i.qty||0)+retailQtyStep(i.product_id)):i.qty+1;if(b.dataset.a==='minus'){i.qty=isRetailProfile()?retailQtyNormalize(i.product_id,Number(i.qty||0)-retailQtyStep(i.product_id)):i.qty-1;if(i.qty<=0)state.cart.splice(+b.dataset.i,1)}if(b.dataset.a==='editqty'&&isRetailProfile()){const raw=prompt('الكمية/الوزن',String(i.qty));if(raw!==null){const q=retailQtyNormalize(i.product_id,Number(raw));if(q>0)i.qty=q}}if(b.dataset.a==='del')state.cart.splice(+b.dataset.i,1);drawCart()};const c=cartCalc();$('#subtotal').textContent=money(c.subtotal);if($('#posPromoResult')&&state.activePromo){$('#posPromoResult').classList.remove('hidden');$('#posPromoResult').innerHTML=`✅ ${esc(state.activePromo.code)} — خصم ${money(c.promoDiscount)} <button type="button" class="link-btn" id="removePosPromo">إلغاء</button>`;const rb=$('#removePosPromo');if(rb)rb.onclick=()=>{invalidateActivePromo();drawCart()}}if($('#deliveryFee'))$('#deliveryFee').textContent=money(c.deliveryFee);if($('#taxAmount'))$('#taxAmount').textContent=money(c.taxAmount);if($('#serviceAmount'))$('#serviceAmount').textContent=money(c.serviceAmount);if($('#retailOfferAmount'))$('#retailOfferAmount').textContent=c.retailOfferDiscount?'-'+money(c.retailOfferDiscount):money(0);if($('#retailOfferLine'))$('#retailOfferLine').classList.toggle('hidden',!c.retailOfferDiscount);if($('#taxLine'))$('#taxLine').classList.toggle('hidden',!financialCfg().tax_enabled);if($('#serviceLine'))$('#serviceLine').classList.toggle('hidden',!financialCfg().service_enabled);$('#grand').textContent=money(c.total)}
function validatedDeliveryCheckoutInput(){
 const rawPhone=($('#customerPhone')?.value||'').trim();
 const phone=normalizePhone(rawPhone);
 const address=($('#deliveryAddress')?.value||'').trim();
 const zoneId=Number($('#deliveryZone')?.value||0);
 if(!/^01[0125][0-9]{8}$/.test(phone))throw new Error('رقم موبايل الدليفري غير صحيح — اكتب رقم مصري 11 رقم');
 if(!address)throw new Error('عنوان التوصيل مطلوب لأوردر الدليفري');
 const zone=(state.deliveryZones||[]).find(z=>Number(z.id)===zoneId&&String(z.branch_id)===String(currentBranchId()));
 if(!zone)throw new Error('اختر منطقة توصيل صحيحة لنفس الفرع');
 return {phone,address,zoneId,area:String(zone.name||'').trim()||null};
}
 ...
 try{
  ...
  if(phone && !customerId){
    try{
      ...
      if(typeof createdCustomer==='string'&&createdCustomer.startsWith('offline-customer-')){customerId=null;customerCreateTx=createdCustomer.slice('offline-customer-'.length)||null}else if(createdCustomer&&typeof createdCustomer==='object'&&createdCustomer.offline===true){customerId=null;customerCreateTx=String(createdCustomer.client_tx_id||'').trim()||null}else{customerId=createdCustomer}
  ...
  const orderPayload={
    ...
    delivery_zone_id:deliveryInput?.zoneId||null,
  ...
  let itemPayload=state.cart.map(i=>({
  ...
  let result;const clientTx=point4UuidV4(),canonical=point4SaleIdentity(clientTx,itemPayload);itemPayload=canonical.lines;Object.assign(orderPayload,{document_uid:canonical.document_uid,source_document_id:canonical.source_document_id,point4_identity_contract:canonical.contract});let localFirst=null;if(window.topBurgerDesktop?.isDesktop){if(state.activePromo&&!navigator.onLine)throw new Error('البرومو كود يحتاج إنترنت. ألغِ البرومو وأكمل البيع أوفلاين');localFirst=await saveOfflineSale(orderPayload,itemPayload,payRows,clientTx)}try{if(!navigator.onLine)throw new TypeError('Failed to fetch');result=await rpc(isRetailProfile()?'create_retail_pos_order_atomic':'create_pos_order_atomic',{p_order:{...orderPayload,client_tx_id:clientTx},p_items:itemPayload,p_payments:payRows});if(window.topBurgerDesktop?.isDesktop)await removeQueuedOperation(clientTx)}catch(err){if(!isNetError(err)){if(window.topBurgerDesktop?.isDesktop){try{await window.topBurgerDesktop.operations.status(clientTx,'failed',String(err.message||err))}catch{}try{await removeQueuedOperation(clientTx)}catch{}}throw err}if(state.activePromo)throw new Error('البرومو كود يحتاج إنترنت. ألغِ البرومو وأكمل البيع أوفلاين');result=localFirst||await saveOfflineSale(orderPayload,itemPayload,payRows,clientTx);toast('تم حفظ الفاتورة محليًا وستتزامن عند رجوع النت')}
  const o=result?.order, savedItems=result?.items||[];
  if(!o?.id)throw new Error('تمت العملية لكن تعذر قراءة الفاتورة');
  await point4RememberOrderIdentity(o,canonical);
  state.cart=[];state.selectedCustomer=null;state.activePromo=null;document.getElementById('receiptPrintFrame')?.remove();toast(officialNumberPending(o)?`تم الحفظ محليًا — المرجع ${bonDisplay(o)}`:`تم حفظ بون ${bonDisplay(o)}`);renderPOS();showReceipt(o,savedItems);
 }catch(e){
  console.error('atomic checkout',e);
  const msg=String(e?.message||'تعذر حفظ الفاتورة');
  if(msg.includes('create_retail_pos_order_atomic'))toast('شغّل SQL beta.15 Retail Inventory أولًا');else if(msg.includes('create_pos_order_atomic')||msg.includes('Could not find the function'))toast('شغّل SQL V9.7.1 أولًا');
  else toast(msg);
...
function receiptIdentityHTML(o,c){const b=branchById(o.branch_id),pending=officialNumberPending(o),ref=localOrderReference(o);return `${c.show_logo&&state.business?.logo_url?`<img class="receipt-logo" src="${esc(state.business.logo_url)}" alt="">`:''}${c.show_business_name?`<h2>${esc(businessName())}</h2>`:''}<div class="r-meta">${c.receipt_header?`<b>${esc(c.receipt_header)}</b><br>`:''}<b>${esc(branchName(o.branch_id))}</b>${c.show_branch_phone&&b.phone?`<br>☎ ${esc(b.phone)}`:''}${c.show_branch_address&&b.address?`<br>${esc(b.address)}`:''}<br><b>${pending?'مرجع محلي':'رقم البون'}: ${esc(pending?(ref||bonDisplay(o)):bonDisplay(o))}</b>${pending?'<br><small>الرقم الرسمي يُعتمد بعد المزامنة</small>':''}<br>${fmtDate(o.created_at||new Date().toISOString())}</div>`;}
...
function prepReceiptHTML(o,items,isCopy=false){const c=printCfg(o.branch_id);return `<div class="receipt">${receiptCopyMark(isCopy)}<div class="receipt-head"><h2>ريسيت التحضير</h2><h1>${officialNumberPending(o)?'مرجع محلي':'بون'} ${esc(bonDisplay(o))}</h1><p>${branchName(o.branch_id)} • ${fmtDate(o.created_at)}</p><p>${orderTypeLabel(o.order_type)}</p></div><hr>${customerInfoHTML(o)}<div class="r-items">${items.map(i=>`<div class="prep-item"><span><b>${i.quantity} × ${esc(i.product_name)}</b>${i.notes?`<small class="prep-note">📝 ${esc(i.notes)}</small>`:''}</span><b>${money(i.total)}</b></div>`).join('')}</div><hr><div class="r-totals"><div><span>إجمالي الأصناف</span><b>${money(o.subtotal)}</b></div></div></div>`}
...
function showReceipt(o,items){const c=printCfg(o.branch_id),autoPrep=localAutoPrint(o.branch_id,'prep',c.auto_print_prep),autoCustomer=localAutoPrint(o.branch_id,'customer',c.auto_print_customer),didAuto=(autoCustomer&&state.settings.enable_receipt_print)||(autoPrep&&state.settings.enable_prep_receipt);autoPrintOrder(o,items);if(didAuto)return;const m=document.createElement('div');m.className='modal';m.innerHTML=`<div class="modal-card"><h2>${officialNumberPending(o)?`تم الحفظ محليًا — ${esc(bonDisplay(o))}`:`تم حفظ بون ${esc(bonDisplay(o))}`}</h2>${receiptHTML(o,items)}<div class="modal-actions"><button class="secondary" data-close>إغلاق</button>${state.settings.enable_prep_receipt?'<button class="secondary" data-prep>ريسيت التحضير</button>':''}${state.settings.enable_receipt_print?'<button class="primary" data-print>فاتورة العميل</button>':''}</div></div>`;document.body.appendChild(m);m.onclick=e=>{if(e.target.closest('[data-close]')||e.target===m)m.remove();if(e.target.closest('[data-print]'))printReceipt(o,items);if(e.target.closest('[data-prep]'))printPrepReceipt(o,items)}}
async function rc1PendingReturnUsage(orderId){
 const used={};let q=[];try{q=await offlineQueue()}catch{}
 for(const j of (q||[])){if(j.type!=='return'||String(j.p_order_id)!==String(orderId))continue;for(const x of (j.p_items||j.local_items||[])){const k=String(x.order_item_id??'');if(k)used[k]=(used[k]||0)+Number(x.quantity||0)}}
 try{const native=await window.topBurgerDesktop?.offlineV2?.outbox?.();for(const n of (native||[])){if(n.operation_type!=='return'||['synced','conflict','dead_letter'].includes(String(n.status||'')))continue;const rp=n.envelope?.payload?.rpc_payload||{};if(String(rp.p_order_id)!==String(orderId))continue;for(const x of (rp.p_items||[])){const k=String(x.order_item_id??'');if(k)used[k]=(used[k]||0)+Number(x.quantity||0)}}}catch{}
 return used;
}
async function rc1PendingSaleLineIndexMap(order){
 const id=String(order?.id||'');if(!id.startsWith('offline-'))return null;
 const parentTx=id.slice('offline-'.length);if(!parentTx)throw new Error('مرجع فاتورة الأوفلاين غير صالح');
 let row=null;try{row=await window.topBurgerDesktop?.offlineV2?.event?.(parentTx)}catch{}
 if(!row)try{const rows=await window.topBurgerDesktop?.offlineV2?.outbox?.();row=(rows||[]).find(x=>String(x.client_tx_id||'')===parentTx)}catch{}
 const rpcName=String(row?.envelope?.payload?.rpc_name||'');
 if(!['create_pos_order_atomic','create_food_pos_order_atomic_v1'].includes(rpcName))throw new Error('مرتجع فاتورة أوفلاين جديدة غير متاح لهذا مسار البيع قبل المزامنة');
 const sourceItems=row?.envelope?.payload?.rpc_payload?.p_items;
 if(!Array.isArray(sourceItems)||!sourceItems.length)throw new Error('تعذر قراءة سطور فاتورة الأوفلاين الأصلية');
 const map=new Map();for(let i=0;i<sourceItems.length;i++){const line=String(sourceItems[i]?.line_uid||'').trim();if(!line||map.has(line))throw new Error('هوية سطر البيع الأوفلاين غير صالحة');map.set(line,i+1)}
 return {parentTx,rpcName,map};
}

async function openReturnForOrder(o,items){
 const profile=currentPosProfile();if(profile!=='restaurant'&&profile!=='retail')return toast(`المرتجعات لهذا النشاط (${profile||'غير محدد'}) تحتاج مالك مخصص ولا تستخدم مسار Restaurant/Retail العام.`);
 const localPendingSale=String(o?.id||'').startsWith('offline-');if(localPendingSale&&profile!=='restaurant')return toast('مرتجع فاتورة Retail أوفلاين جديدة يتم بعد مزامنة الفاتورة أولًا');
 const pendingSaleLineMap=localPendingSale?await rc1PendingSaleLineIndexMap(o):null;
 await point4HydrateOrderIdentity(o);
 ...
 o._offline_return_shift_id=sh.id;
 ...
 const used={};for(const x of prior)used[String(x.order_item_id)]=(used[String(x.order_item_id)]||0)+Number(x.quantity||0);const pendingUsed=await rc1PendingReturnUsage(o.id);for(const [k,v] of Object.entries(pendingUsed))used[k]=(used[k]||0)+Number(v||0);
 ...
 m.onclick=async e=>{if(e.target.closest('[data-close]')||e.target===m){m.remove();return}if(e.target.closest('[data-full-return]')){for(const inp of m.querySelectorAll('.return-qty')){const i=available.find(x=>String(x.id)===String(inp.dataset.ri));inp.value=i?i._available:0}calc();toast('تم تحديد كل الكميات المتاحة للمرتجع');return}if(!e.target.closest('[data-submit-return]'))return;const selected=[...m.querySelectorAll('.return-qty')].map(inp=>{const item=available.find(x=>String(x.id)===String(inp.dataset.ri));const rawQty=Number(inp.value||0)||0;const normalized=isRetailProfile()?Math.round(rawQty*1000)/1000:Math.floor(rawQty);const quantity=Math.min(Number(item?._available||0),Math.max(0,normalized));inp.value=quantity;if(!localPendingSale)return {order_item_id:Number(inp.dataset.ri),quantity};const saleLineUid=String(item?.line_uid||'').trim(),sourceIndex=pendingSaleLineMap?.map?.get(saleLineUid)||0;return {order_item_id:String(inp.dataset.ri),source_order_item_index:sourceIndex,source_sale_line_uid:saleLineUid,quantity}}).filter(x=>x.quantity>0);if(localPendingSale&&selected.some(x=>!Number.isInteger(Number(x.source_order_item_index))||Number(x.source_order_item_index)<=0||!x.source_sale_line_uid))return toast('تعذر ربط سطر المرتجع بسطر البيع المحلي — لم يتم تسجيل المرتجع');if(!selected.length)return toast('حدد كمية مرتجع لصنف واحد على الأقل');const latestPending=await rc1PendingReturnUsage(o.id);for(const x of selected){const item=items.find(i=>String(i.id)===String(x.order_item_id));const serverUsed=prior.filter(r=>String(r.order_item_id)===String(x.order_item_id)).reduce((a,r)=>a+Number(r.quantity||0),0);const remaining=Math.max(0,Number(item?.quantity||0)-serverUsed-Number(latestPending[String(x.order_item_id)]||0));if(Number(x.quantity)>remaining)return toast('الكمية المطلوبة محجوزة بالفعل في مرتجع محلي معلق — حدّث الفاتورة وحاول مرة أخرى')}const reason=m.querySelector('#retReason').value;if(!reason)return toast('اختر سبب المرتجع');const total=calc();if(total<=0)return toast('قيمة المرتجع غير صحيحة');if(!await uiConfirm(`تأكيد مرتجع ${money(total)}؟\nالعملية لن تمسح الفاتورة الأصلية.`,{title:'تأكيد المرتجع',danger:true,okText:'تنفيذ المرتجع'}))return;const btn=m.querySelector('[data-submit-return]');btn.disabled=true;try{let rr,ri,rp,rid;const returnTx=point4UuidV4(),returnIdentity=point4ReturnIdentity(returnTx,o,selected),canonicalItems=returnIdentity.lines;try{rid=Number(await rpc(isRetailProfile()?'create_retail_order_return_idempotent':'create_order_return_idempotent',{p_order_id:localPendingSale?String(o.id):Number(o.id),p_reason:reason,p_notes:m.querySelector('#retNotes').value.trim()||null,p_items:canonicalItems,p_payments:[{method:m.querySelector('#retMethod').value,amount:total}],p_client_tx_id:returnTx}));[rr,ri,rp]=await Promise.all([rest('returns',`select=*&id=eq.${rid}`),rest('return_items',`select=*&return_id=eq.${rid}&order=id`),rest('return_payments',`select=*&return_id=eq.${rid}&order=id`)])}catch(netErr){if(!isNetError(netErr))throw netErr;const saved=await saveOfflineReturn(o,canonicalItems,reason,m.querySelector('#retNotes').value.trim()||null,m.querySelector('#retMethod').value,total,available,returnTx,returnIdentity);rr=[saved.r];ri=saved.items;rp=saved.payments;rid=saved.r.id}m.remove();toast(rr[0]?._offline?'تم حفظ المرتجع أوفلاين وسيُزامن تلقائيًا':`تم تسجيل مرتجع #${rr[0]?.return_number||rid}`);const x=document.createElement('div');x.className='modal';x.innerHTML=`<div class="modal-card"><h2>تم تسجيل المرتجع</h2>${returnReceiptHTML(rr[0],ri,rp)}<div class="modal-actions"><button class="secondary" data-close>إغلاق</button><button class="primary" data-print-return>🖨️ طباعة إيصال المرتجع</button></div></div>`;document.body.appendChild(x);x.onclick=z=>{if(z.target.closest('[data-close]')||z.target===x)x.remove();if(z.target.closest('[data-print-return]'))printReturnReceipt(rr[0],ri,rp)}}catch(err){btn.disabled=false;toast(err.message)}};
}
...
function ordersDateValue(d=new Date()){const x=new Date(d.getTime()-d.getTimezoneOffset()*60000);return x.toISOString().slice(0,10)}
function ordersDayBounds(value,end=false){const d=new Date(`${value}T${end?'23:59:59.999':'00:00:00.000'}`);return d.toISOString()}
async function cacheOrderRows(rows){if(!rows?.length)return;const all=await cachedOrderBundles(),byId=new Map(all.map(x=>[String(x.order?.id),x]));for(const o of rows){const key=String(o.id),old=byId.get(key);byId.set(key,{order:o,items:old?.items||[]})}const next=[...byId.values()].sort((a,b)=>String(b.order?.created_at||'').localeCompare(String(a.order?.created_at||''))).slice(0,250);await odbSet('cachedOrders',next)}
async function renderOrders(opts={}){
  // Runtime-visible marker: proves the installed package is executing this renderer, not a stale/overridden Orders UI.
  const runtimeMarker='ORDERS-DATE-WINDOW-V58.3';const today=ordersDateValue(),from=opts.from||today,to=opts.to||from,page=Math.max(1,Number(opts.page)||1),pageSize=100,offset=(page-1)*pageSize;let rows=[],online=true;try{const fromIso=encodeURIComponent(ordersDayBounds(from)),toIso=encodeURIComponent(ordersDayBounds(to,true));rows=await rest('orders',`select=*&branch_id=eq.${currentBranchId()}&created_at=gte.${fromIso}&created_at=lte.${toIso}&order=created_at.desc&limit=${pageSize+1}&offset=${offset}`);await cacheOrderRows(rows.slice(0,pageSize))}catch(e){if(!isNetError(e))throw e;online=false;rows=(await cachedOrderBundles()).map(x=>x.order).filter(o=>Number(o.branch_id)===currentBranchId()&&String(o.created_at||'')>=ordersDayBounds(from)&&String(o.created_at||'')<=ordersDayBounds(to,true)).sort((a,b)=>String(b.created_at||'').localeCompare(String(a.created_at||''))).slice(offset,offset+pageSize+1)}const hasNext=rows.length>pageSize;rows=rows.slice(0,pageSize);$('#page').innerHTML=`<div class="panel" data-orders-runtime="${runtimeMarker}"><div class="hint" style="margin-bottom:8px">Orders Runtime: ${runtimeMarker}</div><h2>الطلبات</h2><div class="grid2"><label>من تاريخ<input id="ordersFrom" type="date" value="${esc(from)}"></label><label>إلى تاريخ<input id="ordersTo" type="date" value="${esc(to)}"></label></div><div class="actions" style="margin-bottom:12px"><button class="primary" id="ordersSearch">بحث</button><button class="secondary" id="ordersToday">اليوم</button></div>${online?'':'<p class="hint">وضع Offline: النتائج من الفواتير المحفوظة على هذا الجهاز فقط.</p>'}<div class="table-wrap"><table><thead><tr><th>رقم الفاتورة</th><th>رقم البون</th><th>التاريخ</th><th>الفرع</th><th>العميل</th><th>النوع</th><th>الدفع</th><th>الحالة</th><th>الإجمالي</th><th></th></tr></thead><tbody>${rows.map(o=>`<tr><td>${esc(invoiceDisplay(o))}</td><td>${esc(bonDisplay(o))}</td><td>${fmtDate(o.created_at)}</td><td>${branchName(o.branch_id)}</td><td>${esc(o.customer_name||o.customer_phone||'-')}</td><td>${orderTypeLabel(o.order_type)}</td><td>${paymentLabel(o.payment_method)}</td><td><span class="tag">${statusLabel(o.status)}</span></td><td>${money(o.total)}</td><td><button class="secondary" data-order="${o.id}">تفاصيل</button></td></tr>`).join('')}</tbody></table></div><div class="actions" style="margin-top:12px"><button class="secondary" id="ordersPrev" ${page<=1?'disabled':''}>السابق</button><span class="tag">صفحة ${page}</span><button class="secondary" id="ordersNext" ${hasNext?'':'disabled'}>التالي</button></div></div>`;const rerun=p=>{const f=$('#ordersFrom').value,t=$('#ordersTo').value;if(!f||!t)return toast('حدد التاريخ');if(f>t)return toast('تاريخ البداية يجب أن يكون قبل تاريخ النهاية');renderOrders({from:f,to:t,page:p})};$('#ordersSearch').onclick=()=>rerun(1);$('#ordersToday').onclick=()=>renderOrders({from:today,to:today,page:1});$('#ordersPrev').onclick=()=>page>1&&rerun(page-1);$('#ordersNext').onclick=()=>hasNext&&rerun(page+1);$('#page').onclick=e=>{const b=e.target.closest('[data-order]');if(b)openOrderDetails(b.dataset.order)}}
...
async function openCustomerImport(){
 ...
 m.onclick=async e=>{if(e.target.closest('[data-close]')||e.target===m){m.remove();return}if(!e.target.closest('[data-import]'))return;btn.disabled=true;try{let added=0,updated=0,skipped=0;const inserts=[];for(const x of prepared){if(!x.existing){inserts.push(x.data);continue}const patch={};if(!x.existing.name&&x.data.name)patch.name=x.data.name;if(!x.existing.area&&x.data.area)patch.area=x.data.area;if(!x.existing.address&&x.data.address)patch.address=x.data.address;if(Object.keys(patch).length){patch.updated_at=new Date().toISOString();const editRouter=globalThis.__SharawlaPV2CustomerEditAddress;if(typeof editRouter?.updateCustomer!=='function')throw new Error('مسار تعديل العميل غير جاهز');await editRouter.updateCustomer({id:x.existing.id,name:patch.name||x.existing.name,phone:x.existing.phone,area:patch.area??x.existing.area,address:patch.address??x.existing.address,notes:x.existing.notes||null});updated++}else skipped++}for(let i=0;i<inserts.length;i+=100){const part=inserts.slice(i,i+100);const createRouter=globalThis.__SharawlaPV2CustomerCreate;if(typeof createRouter?.createManyCustomers!=='function')throw new Error('مسار إنشاء العميل غير جاهز');await createRouter.createManyCustomers(part);added+=part.length}toast(`تم الاستيراد: ${added} جديد، ${updated} تحديث، ${skipped} موجود`);m.remove();renderCustomers()}catch(err){btn.disabled=false;toast('تعذر الاستيراد: '+err.message)}};
}
...
async function localShiftProjectionRows(table,shiftId){
 ...
 if(table==='orders'){
  ...
  for(const j of await offlineQueue()){if(j.type==='expense'&&String(j.local_expense?.shift_id)===sid)rows.push(j.local_expense)}
 }else if(table==='returns'){
  for(const j of await offlineQueue()){if(j.type==='return'&&String(j.local_return?.shift_id)===sid)rows.push(j.local_return)}
 }
 return rows;
}
...
async function shiftReportData(shift){
 if(!isServerShiftId(shift?.id)){
  const m=await localShiftMetrics(shift);
...
async function renderWebsitePayments(){
 ...
 const draw=()=>{
   ...
   $('#saveWebsitePayments').onclick=async()=>{try{await rc1RequireCloudOnline('حفظ طرق دفع الموقع');const rows=state.paymentMethods.map(m=>{const existing=state.branchPaymentMethods.find(x=>Number(x.branch_id)===activeBranch&&Number(x.payment_method_id)===Number(m.id));return {branch_id:activeBranch,payment_method_id:Number(m.id),active:existing?.active!==false,is_default:existing?.is_default===true,website_enabled:$(`[data-webpay-enable="${m.id}"]`)?.checked===true,payment_account:$(`[data-webpay-account="${m.id}"]`)?.value.trim()||null,payment_instructions:$(`[data-webpay-instructions="${m.id}"]`)?.value.trim()||null,allow_reference:$(`[data-webpay-ref="${m.id}"]`)?.checked!==false,allow_receipt_upload:$(`[data-webpay-receipt="${m.id}"]`)?.checked!==false}});if(rows.length)await rest('branch_payment_methods','',{method:'POST',headers:{Prefer:'resolution=merge-duplicates'},body:JSON.stringify(rows)});state.branchPaymentMethods=await rest('branch_payment_methods','select=*');toast('تم حفظ طرق الدفع على الموقع')}catch(e){toast(e.message)}};
 };
 draw();
}
...
async function uploadProductImage(product,file){
 ...
 }catch(e){if(uploaded){try{await rc1RemoveStorageObject('product-images',path)}catch(cleanErr){throw new Error(`${e.message||e} — وتعذر تنظيف الملف غير المرتبط: ${cleanErr.message||cleanErr}`)}}throw e}
}
...
async function renderInventory(){
 // INVENTORY-OVERVIEW-V1-CORE-DELEGATE
 const runtime=window.__SharawlaInventoryOverviewV1;
 if(runtime?.render)return runtime.render();
 $('#page').innerHTML='<div class="panel"><h2>📦 المخزون</h2><div class="empty">Inventory Overview runtime غير محمل — تم إيقاف المسار بأمان.</div></div>';
}
function retailQty(v){const n=Number(v||0);return Number.isFinite(n)?n.toFixed(3):'0.000'}
async function renderRetailInventory(){
 if(!moduleEnabled('inventory')){$('#page').innerHTML='<div class="empty">موديول المخزون غير مفعّل لهذا النشاط</div>';return;}
 const branchId=Number(currentBranchId());
 let balances=[],settingsRows=[],movements=[];
 try{
  [balances,settingsRows,movements]=await Promise.all([
   rest('retail_inventory_balances',`select=*&branch_id=eq.${branchId}`),
   rest('retail_inventory_settings',`select=*&branch_id=eq.${branchId}`),
   rest('retail_inventory_movements',`select=*&branch_id=eq.${branchId}&order=created_at.desc&limit=60`)
  ]);
 }catch(e){
  if(String(e?.message||'').includes('retail_inventory_')){$('#page').innerHTML='<div class="empty">شغّل SQL beta.15 Retail Inventory على قاعدة الـBeta أولًا</div>';return;}
  throw e;
 }
 const settingsRow=settingsRows[0]||{allow_negative_stock:false,default_low_stock_threshold:0};
 const products=state.products.filter(p=>p.active!==false&&productAvailableAtBranch(p));
 const byProduct=new Map(balances.map(x=>[String(x.product_id),x]));
 const thresholdFor=r=>Number(r?.low_stock_threshold??settingsRow.default_low_stock_threshold??0);
 const lowCount=products.filter(p=>{const r=byProduct.get(String(p.id));return (r?.track_inventory??true)&&Number(r?.quantity||0)<=thresholdFor(r)}).length;
 $('#page').innerHTML=`<div class="panel"><div class="section-head"><div><h2>📦 مخزون Retail</h2><p class="muted">الرصيد لكل فرع بدقة 3 منازل عشرية. البيع والمرتجع يسجلان حركة تلقائية.</p></div><div class="chips"><span class="chip">الأصناف ${products.length}</span><span class="chip">منخفض ${lowCount}</span></div></div>
 <div class="form-grid"><label>الحد الافتراضي للتنبيه<input id="retDefaultLow" type="number" step="0.001" min="0" value="${retailQty(settingsRow.default_low_stock_threshold)}"></label><label class="setting-switch"><span>السماح بالبيع بالسالب</span><input id="retAllowNegative" type="checkbox" ${settingsRow.allow_negative_stock?'checked':''}></label></div><button id="saveRetailInventoryPolicy" class="secondary">حفظ سياسة المخزون</button></div>
 <div class="panel"><h2>أرصدة الأصناف</h2><div class="table-wrap"><table><thead><tr><th>الصنف</th><th>الباركود</th><th>الرصيد</th><th>حد التنبيه</th><th>تتبع</th><th>حركة</th></tr></thead><tbody>${products.map(p=>{const r=byProduct.get(String(p.id))||{};const low=(r.track_inventory??true)&&Number(r.quantity||0)<=thresholdFor(r);return `<tr><td>${esc(p.name)}</td><td>${esc(p.barcode||'-')}</td><td class="${low?'negative':''}"><b>${retailQty(r.quantity)}</b></td><td><input data-low="${p.id}" type="number" min="0" step="0.001" value="${retailQty(r.low_stock_threshold??settingsRow.default_low_stock_threshold)}" style="max-width:110px"></td><td><input data-track="${p.id}" type="checkbox" ${r.track_inventory===false?'':'checked'}></td><td><div class="chips"><button class="secondary" data-stock="${p.id}" data-stock-type="opening">افتتاحي</button><button class="secondary" data-stock="${p.id}" data-stock-type="adjustment">تسوية</button><button class="secondary" data-stock="${p.id}" data-stock-type="waste">فاقد</button></div></td></tr>`}).join('')||'<tr><td colspan="6">لا توجد أصناف</td></tr>'}</tbody></table></div></div>
 <div class="panel"><h2>آخر حركات المخزون</h2><div class="table-wrap"><table><thead><tr><th>الوقت</th><th>الصنف</th><th>النوع</th><th>الحركة</th><th>الرصيد بعد</th><th>ملاحظة</th></tr></thead><tbody>${movements.map(m=>`<tr><td>${fmtDate(m.created_at)}</td><td>${esc(state.products.find(p=>String(p.id)===String(m.product_id))?.name||m.product_id)}</td><td>${esc(m.movement_type)}</td><td class="${Number(m.quantity_delta)<0?'negative':'positive'}">${Number(m.quantity_delta)>0?'+':''}${retailQty(m.quantity_delta)}</td><td>${retailQty(m.balance_after)}</td><td>${esc(m.notes||'-')}</td></tr>`).join('')||'<tr><td colspan="6">لا توجد حركات حتى الآن</td></tr>'}</tbody></table></div></div>`;
 $('#saveRetailInventoryPolicy').onclick=async()=>{try{await rpc('retail_inventory_set_policy',{p_branch_id:branchId,p_allow_negative_stock:$('#retAllowNegative').checked,p_default_low_stock_threshold:Number($('#retDefaultLow').value||0)});toast('تم حفظ سياسة المخزون');renderRetailInventory()}catch(e){toast(e.message)}};
 $('#page').onclick=async e=>{
  const stock=e.target.closest('[data-stock]');if(stock){const productId=Number(stock.dataset.stock),p=state.products.find(x=>Number(x.id)===productId),type=stock.dataset.stockType||'adjustment';const title=type==='opening'?'رصيد افتتاحي':type==='waste'?'تسجيل فاقد':'تسوية مخزون';const hint=type==='adjustment'?'اكتب موجب للزيادة أو سالب للنقص':'اكتب الكمية بالموجب';const qty=await uiPrompt(`${hint}\n${p?.name||''}`,'0',{title,type:'number',okText:'تنفيذ'});if(qty===null)return;let delta=Math.round(Number(qty||0)*1000)/1000;if(!Number.isFinite(delta)||delta===0)return toast('اكتب كمية صحيحة غير صفر');if(type!=='adjustment')delta=Math.abs(delta);const note=await uiPrompt('ملاحظة الحركة','',{title:'ملاحظة المخزون'});if(note===null)return;try{await rpc('retail_inventory_adjust',{p_branch_id:branchId,p_product_id:productId,p_quantity_delta:delta,p_movement_type:type,p_notes:note||null,p_client_tx_id:uuid()});toast('تم تسجيل حركة المخزون');return renderRetailInventory()}catch(err){return toast(err.message)}}
  const track=e.target.closest('[data-track]');if(track){const productId=Number(track.dataset.track),low=$(`[data-low="${productId}"]`);try{await rpc('retail_inventory_set_item_policy',{p_branch_id:branchId,p_product_id:productId,p_track_inventory:track.checked,p_low_stock_threshold:Number(low?.value||0)});toast('تم تحديث إعداد الصنف')}catch(err){track.checked=!track.checked;toast(err.message)}return}
...
async function renderRetailMarketSettings(){
 if(!isRetailProfile()){$('#page').innerHTML='<div class="empty">إعدادات الماركت متاحة لقطاع Retail فقط</div>';return;}
 await loadRetailMarketData(true);
 const rows=state.products.filter(p=>p.active!==false&&productAvailableAtBranch(p));
 $('#page').innerHTML=`<div class="panel"><div class="section-head"><div><h2>⚖️ وحدات القياس وباركود الوزن</h2><p class="muted">Piece / Kg / Liter + الكميات العشرية + باركود الميزان EAN-13. الصيغة: Prefix(2)+PLU(5)+Value(5)+Check.</p></div></div><div class="table-wrap"><table><thead><tr><th>الصنف</th><th>الوحدة</th><th>عشري</th><th>خطوة</th><th>نوع الباركود</th><th>Prefix</th><th>PLU</th><th>Online</th><th>إجراء</th></tr></thead><tbody>${rows.map(p=>{const x=retailProductSetting(p.id);return `<tr><td>${esc(p.name)}<small>${p.barcode?`<br>${esc(p.barcode)}`:''}</small></td><td>${esc(x.unit_type||'piece')}</td><td>${x.allow_decimal?'✅':'—'}</td><td>${retailQty(x.qty_step)}</td><td>${esc(x.barcode_mode||'normal')}</td><td>${esc(x.weight_prefix||'—')}</td><td>${esc(x.plu_code||'—')}</td><td>${x.online_enabled!==false?'✅':'⛔'}</td><td><button class="secondary" data-market-product="${p.id}">تعديل</button></td></tr>`}).join('')}</tbody></table></div></div>`;
 $('#page').onclick=async e=>{const b=e.target.closest('[data-market-product]');if(!b)return;const p=state.products.find(x=>String(x.id)===String(b.dataset.marketProduct));if(!p)return;const cur=retailProductSetting(p.id);const unit=await uiPrompt('الوحدة: piece / kg / g / liter / ml',cur.unit_type||'piece',{title:p.name});if(unit===null)return;const decimal=await uiConfirm('هل الصنف يسمح بكميات عشرية؟',{title:'الكميات العشرية',okText:'نعم'});const step=await uiPrompt('خطوة الكمية (مثال 0.001 للوزن)',String(cur.qty_step||1),{type:'number',title:'خطوة الكمية'});if(step===null)return;const mode=await uiPrompt('نوع الباركود: normal / weight / price',cur.barcode_mode||'normal',{title:'باركود الميزان'});if(mode===null)return;let prefix=cur.weight_prefix||'',plu=cur.plu_code||'',divisor=cur.embedded_divisor||1000;if(['weight','price'].includes(String(mode).trim())){const a=await uiPrompt('Prefix أول رقمين (مثال 20)',prefix||'20',{title:'باركود الميزان'});if(a===null)return;prefix=a.trim();const z=await uiPrompt('PLU خمس أرقام للصنف',plu||String(p.id).padStart(5,'0').slice(-5),{title:'باركود الميزان'});if(z===null)return;plu=z.trim();const d=await uiPrompt('المقسوم عليه للقيمة المدمجة (1000 للجرام/القيمة بالملّي)',String(divisor),{type:'number',title:'باركود الميزان'});if(d===null)return;divisor=Number(d)}const online=await uiConfirm('هل الصنف متاح للبيع أونلاين؟',{title:'الموقع',okText:'متاح'});try{await rpc('retail_set_product_settings',{p_product_id:Number(p.id),p_unit_type:String(unit).trim(),p_allow_decimal:decimal,p_qty_step:Number(step),p_min_qty:Number(step),p_barcode_mode:String(mode).trim(),p_weight_prefix:prefix||null,p_plu_code:plu||null,p_embedded_divisor:Number(divisor)||1000,p_online_enabled:online});retailMarket.loadedBranch=null;toast('تم حفظ إعدادات الصنف');renderRetailMarketSettings()}catch(err){toast(err.message)}};
}

async function renderRetailOffers(){
 if(!isRetailProfile()){$('#page').innerHTML='<div class="empty">عروض الماركت متاحة لقطاع Retail فقط</div>';return;}
 await loadRetailMarketData(true);const offers=retailMarket.offers,ops=retailMarket.offerProducts;
 const productNames=id=>ops.filter(x=>String(x.offer_id)===String(id)).map(x=>state.products.find(p=>String(p.id)===String(x.product_id))?.name||x.product_id).join('، ')||'كل الأصناف';
 $('#page').innerHTML=`<div class="panel"><div class="section-head"><div><h2>🏷️ Offers Engine</h2><p class="muted">نسبة / مبلغ ثابت / اشتري X وخد Y / الثاني بنصف السعر. الأولوية الأقل تُطبق أولًا، ويتم اختيار أول عرض مطابق لكل صنف.</p></div><button id="addRetailOffer" class="primary">➕ عرض جديد</button></div><div class="table-wrap"><table><thead><tr><th>#</th><th>العرض</th><th>القاعدة</th><th>القيمة</th><th>الأصناف</th><th>الفرع</th><th>Online</th></tr></thead><tbody>${offers.map(o=>`<tr><td>${o.id}</td><td>${esc(o.name)}</td><td>${esc(o.rule_type)}</td><td>${o.rule_type==='buy_x_get_y'?`${o.buy_qty}+${o.get_qty}`:o.value}</td><td>${esc(productNames(o.id))}</td><td>${o.branch_id?esc(branchName(o.branch_id)):'كل الفروع'}</td><td>${o.website_enabled!==false?'✅':'⛔'}</td></tr>`).join('')||'<tr><td colspan="7">لا توجد عروض</td></tr>'}</tbody></table></div></div>`;
 $('#addRetailOffer').onclick=async()=>{const name=await uiPrompt('اسم العرض','',{title:'عرض جديد'});if(name===null||!name.trim())return;const type=await uiPrompt('النوع: percent / fixed / buy_x_get_y / second_half','percent',{title:'نوع العرض'});if(type===null)return;let value=0,buy=null,get=null;if(type==='buy_x_get_y'){const x=await uiPrompt('اشترِ كام؟','2',{type:'number'});if(x===null)return;const y=await uiPrompt('وخد كام مجانًا؟','1',{type:'number'});if(y===null)return;buy=Number(x);get=Number(y)}else if(type!=='second_half'){const x=await uiPrompt(type==='percent'?'نسبة الخصم %':'الخصم لكل وحدة','10',{type:'number'});if(x===null)return;value=Number(x)}const ids=await uiPrompt('أرقام الأصناف مفصولة بفاصلة — اتركها فارغة لتطبيق العرض على كل الأصناف\n'+state.products.filter(p=>p.active!==false).map(p=>`${p.id}:${p.name}`).join(' | '),'',{title:'الأصناف'});if(ids===null)return;const productIds=String(ids).split(/[,، ]+/).map(Number).filter(Number.isFinite);const website=await uiConfirm('تفعيل العرض على الموقع أيضًا؟',{title:'الموقع',okText:'نعم'});try{await rpc('retail_offer_save',{p_id:null,p_name:name.trim(),p_rule_type:String(type).trim(),p_value:Number(value)||0,p_buy_qty:buy,p_get_qty:get,p_priority:100,p_starts_at:null,p_ends_at:null,p_branch_id:Number(currentBranchId()),p_website_enabled:website,p_product_ids:productIds});retailMarket.loadedBranch=null;toast('تم حفظ العرض');renderRetailOffers()}catch(e){toast(e.message)}};
}

async function renderRetailStockCount(){
 if(!isRetailProfile()){$('#page').innerHTML='<div class="empty">الجرد متاح لقطاع Retail فقط</div>';return;}
 const branchId=Number(currentBranchId());let balances=[];try{balances=await rest('retail_inventory_balances',`select=*&branch_id=eq.${branchId}`)}catch(e){return toast(e.message)}const map=new Map(balances.map(x=>[String(x.product_id),x]));const products=state.products.filter(p=>p.active!==false&&productAvailableAtBranch(p));
 $('#page').innerHTML=`<div class="panel"><div class="section-head"><div><h2>🧮 جرد المخزون — ${esc(branchName(branchId))}</h2><p class="muted">اكتب الكمية الفعلية. الترحيل ينشئ Adjustment في Inventory Ledger للفروقات فقط.</p></div><button id="postRetailCount" class="primary">✅ ترحيل الجرد</button></div><div class="table-wrap"><table><thead><tr><th>الصنف</th><th>رصيد النظام</th><th>الفعلي</th></tr></thead><tbody>${products.map(p=>{const b=map.get(String(p.id))||{};return `<tr><td>${esc(p.name)}</td><td>${retailQty(b.quantity)}</td><td><input data-count-product="${p.id}" type="number" step="0.001" min="0" value="${Number(b.quantity||0)}"></td></tr>`}).join('')}</tbody></table></div></div>`;
 $('#postRetailCount').onclick=async()=>{if(!await uiConfirm('ترحيل الجرد وتعديل أرصدة المخزون؟',{title:'تأكيد الجرد',danger:true,okText:'ترحيل'}))return;const items=[...document.querySelectorAll('[data-count-product]')].map(x=>({product_id:Number(x.dataset.countProduct),counted_qty:Number(x.value||0)}));const notes=await uiPrompt('ملاحظات الجرد (اختياري)','',{title:'الجرد'});if(notes===null)return;try{const id=await rpc('retail_post_stock_count',{p_branch_id:branchId,p_notes:notes||null,p_items:items});toast(`تم ترحيل الجرد #${id}`);renderRetailInventory()}catch(e){toast(e.message)}};
}

async function renderRetailTransfers(){
 if(!isRetailProfile()){$('#page').innerHTML='<div class="empty">التحويلات متاحة لقطاع Retail فقط</div>';return;}
 const branchId=Number(currentBranchId());let transfers=[],items=[];try{[transfers,items]=await Promise.all([rest('retail_transfers',`select=*&or=(from_branch_id.eq.${branchId},to_branch_id.eq.${branchId})&order=created_at.desc&limit=50`),rest('retail_transfer_items','select=*')])}catch(e){if(String(e.message).includes('retail_transfers')){$('#page').innerHTML='<div class="empty">شغّل SQL beta.17 Retail Market Core أولًا</div>';return}throw e}const products=new Map(state.products.map(p=>[String(p.id),p]));const trItems=id=>items.filter(x=>String(x.transfer_id)===String(id));
 $('#page').innerHTML=`<div class="panel"><div class="section-head"><div><h2>🔄 تحويلات الفروع</h2><p class="muted">الإرسال يخصم من المصدر فورًا، والاستلام يضيف للمستلم بحركة مستقلة في الـLedger.</p></div><button id="newRetailTransfer" class="primary">➕ تحويل جديد</button></div><div class="table-wrap"><table><thead><tr><th>#</th><th>من</th><th>إلى</th><th>الحالة</th><th>الأصناف</th><th>التاريخ</th><th>إجراء</th></tr></thead><tbody>${transfers.map(t=>`<tr><td>${t.id}</td><td>${esc(branchName(t.from_branch_id))}</td><td>${esc(branchName(t.to_branch_id))}</td><td>${esc(t.status)}</td><td>${trItems(t.id).map(i=>`${esc(products.get(String(i.product_id))?.name||i.product_id)} × ${retailQty(i.quantity)}`).join('<br>')}</td><td>${fmtDate(t.created_at)}</td><td>${t.status==='sent'&&Number(t.to_branch_id)===branchId?`<button class="primary" data-receive-transfer="${t.id}">استلام</button>`:'—'}</td></tr>`).join('')||'<tr><td colspan="7">لا توجد تحويلات</td></tr>'}</tbody></table></div></div>`;
 $('#newRetailTransfer').onclick=async()=>{const targets=state.branches.filter(b=>b.active!==false&&Number(b.id)!==branchId);if(!targets.length)return toast('لا يوجد فرع آخر');const to=await uiPrompt('رقم الفرع المستلم:\n'+targets.map(b=>`${b.id} - ${b.name}`).join('\n'),String(targets[0].id),{type:'number',title:'تحويل مخزون'});if(to===null)return;const lines=[];for(const p of state.products.filter(x=>x.active!==false&&productAvailableAtBranch(x))){const q=await uiPrompt(`كمية ${p.name} — 0 للتخطي`,'0',{type:'number',title:'تحويل مخزون'});if(q===null)return;const qty=Math.round(Number(q||0)*1000)/1000;if(qty>0)lines.push({product_id:Number(p.id),quantity:qty})}if(!lines.length)return toast('لم تحدد كميات');const notes=await uiPrompt('ملاحظات التحويل','',{title:'تحويل'});if(notes===null)return;try{await rpc('retail_transfer_create',{p_from_branch_id:branchId,p_to_branch_id:Number(to),p_items:lines,p_notes:notes||null,p_client_tx_id:uuid()});toast('تم إرسال التحويل');renderRetailTransfers()}catch(e){toast(e.message)}};
 $('#page').onclick=async e=>{const b=e.target.closest('[data-receive-transfer]');if(!b)return;if(!await uiConfirm('تأكيد استلام التحويل وإضافة الكميات للمخزون؟'))return;try{await rpc('retail_transfer_receive',{p_transfer_id:Number(b.dataset.receiveTransfer)});toast('تم استلام التحويل');renderRetailTransfers()}catch(err){toast(err.message)}};
}
...
async function uploadBusinessLogo(file){
 ...
 }catch(e){if(uploaded){try{await rc1RemoveStorageObject('business-assets',path)}catch(cleanErr){throw new Error(`${e.message||e} — وتعذر تنظيف اللوجو غير المرتبط: ${cleanErr.message||cleanErr}`)}}throw e}
}
async function renderSettings(){
 ...
 if(canSystem){
   ...
   systemPanel=`<div class="panel" data-settings-section="features"><h2>تشغيل وإيقاف المميزات</h2><p>الإعدادات دي بتتزامن على كل الأجهزة.</p><div class="settings-list">${[['enable_extras','الإضافات Extras'],['enable_removals','حذف مكونات'],['enable_item_notes','ملاحظات على الصنف'],['enable_kitchen','تفعيل تشغيل المطبخ'],['enable_prep_receipt','ريسيت التحضير'],['enable_receipt_print','فاتورة العميل'],['enable_inventory','المخزون'],['enable_delivery','الدليفري'],['enable_customer_search','بحث العملاء بالهاتف'],['enable_delivery_drivers','مناديب التوصيل'],['enable_mixed_payment','الدفع المختلط']].map(([k,l])=>`<label class="setting-switch"><span>${l}</span><input type="checkbox" data-setting="${k}" ${state.settings[k]?'checked':''}></label>`).join('')}</div><button id="saveSettings" class="primary">حفظ الإعدادات</button></div>`;
   backupPanel=`<div class="panel backup-panel" data-settings-section="backup"><h2>💾 النسخ الاحتياطي والاستعادة</h2><p>حدد البيانات المطلوبة. النسخة لا تحتوي كلمات مرور حسابات الدخول.</p><div class="backup-grid">${groups}</div><div class="toolbar"><button id="backupAll" class="secondary">تحديد الكل</button><button id="backupNone" class="secondary">إلغاء التحديد</button><button id="createBackup" class="primary">⬇️ إنشاء Backup</button></div><div class="toolbar"><label class="file-btn secondary">اختر ملف<input id="restoreFile" type="file" accept="application/json,.json"></label><button id="restoreBackup" class="secondary">♻️ استعادة المحدد</button><button id="resetSelected" class="danger">🗑️ إعادة ضبط المحدد</button></div><small>إعادة الضبط لا تحذف حساب المدير من Authentication. استخدمها بحذر وبعد إنشاء Backup.</small></div>`;
 }
 const settingsTabs=[
   canBusiness&&['business','🎨 النشاط'],
...
if('serviceWorker' in navigator){
  window.addEventListener('load',()=>{
    // Desktop Electron uses native Offline V2; never install the PWA service worker there.
    if(window.topBurgerDesktop?.isDesktop){
      navigator.serviceWorker.getRegistrations().then(rows=>Promise.all(rows.map(r=>r.unregister()))).catch(()=>{});
      if('caches' in window)caches.keys().then(keys=>Promise.all(keys.filter(k=>k.startsWith('sharawla-pos-v')).map(k=>caches.delete(k)))).catch(()=>{});
      return;
    }
...
function openDriverPicker(orderId, drivers, onDone){
 ...
 document.body.appendChild(m);
 m.onclick=async e=>{if(e.target===m||e.target.closest('[data-close]'))return m.remove();const b=e.target.closest('[data-driver-pick]');if(!b)return;const d=available.find(x=>String(x.id)===String(b.dataset.driverPick));if(!d)return;try{const router=globalThis.__SharawlaPV2OrderDriverAssignment;if(typeof router?.assignDriver!=='function')throw new Error('مسار تسليم الطلب للمندوب غير جاهز');await router.assignDriver(orderId,d.id);await audit('assign_driver','order',orderId,{driver_id:d.id});m.remove();toast(navigator.onLine===false?`تم حفظ تسليم الطلب إلى ${d.name} للمزامنة`:`تم التسليم إلى ${d.name}`);if(onDone)onDone(d)}catch(err){toast(err.message)}};
}
...
async function openPaymentReceipt(path){if(!path)return toast('لا يوجد إيصال مرفوع');if(!navigator.onLine)return toast('عرض إيصال الدفع يحتاج اتصالًا بالإنترنت. لم يتم تغيير أي بيانات.');try{const r=await fetch(`${cfg.url}/storage/v1/object/authenticated/website-payment-receipts/${encodeURI(path)}`,{headers:{apikey:cfg.key,Authorization:`Bearer ${session.access_token}`}});if(!r.ok)throw new Error('تعذر تحميل الإيصال');const blob=await r.blob(),url=URL.createObjectURL(blob);const m=document.createElement('div');m.className='modal';m.innerHTML=`<div class="modal-card"><h2>🧾 إيصال الدفع</h2><img class="receipt-preview" src="${url}" alt="إيصال الدفع"><div class="modal-actions"><button class="secondary" data-close>إغلاق</button></div></div>`;document.body.appendChild(m);m.onclick=e=>{if(e.target===m||e.target.closest('[data-close]')){URL.revokeObjectURL(url);m.remove()}}}catch(e){toast(isNetError(e)?'عرض إيصال الدفع يحتاج اتصالًا بالإنترنت. لم يتم تغيير أي بيانات.':e.message)}}
...
async function acceptOnlineOrderFromChannel(orderRef,opts={}){
  ...
  if(!sh)throw new Error('افتح وردية أولًا قبل استلام طلب الموقع');
  if(!await openWebsiteOrderReview(websiteOrderId,'accept'))return {accepted:false,cancelled:true};
  const orderId=Number(await rpc('accept_website_order',{p_website_order_id:websiteOrderId}));
  const [orders,items]=await Promise.all([
    rest('orders',`select=*&id=eq.${orderId}`),
    rest('order_items',`select=*&order_id=eq.${orderId}&order=id`)
  ]);
...
async function renderOnlineOrders(opts={}){
  ...
  if(navigator.onLine===false){$('#page').innerHTML='<div class="panel"><h2>🌐 الطلبات الأونلاين</h2><div class="empty"><b>هذه الشاشة تحتاج اتصالًا بالإنترنت.</b><p>لم يتم عرض قائمة فارغة ولم يتم تغيير أي طلب. بعد رجوع الاتصال اضغط على الشاشة مرة أخرى.</p></div></div>';return}
  ...
  let sourceRowsRaw,canonicalRowsRaw;try{[sourceRowsRaw,canonicalRowsRaw]=await Promise.all([
    rest('website_orders',`select=*&branch_id=eq.${currentBranchId()}&created_at=gte.${fromIso}&created_at=lte.${toIso}&order=created_at.desc&limit=${pageSize+1}&offset=${offset}`),
    rest('orders',`select=*&branch_id=eq.${currentBranchId()}&source=eq.website&created_at=gte.${fromIso}&created_at=lte.${toIso}&order=created_at.desc&limit=${pageSize+1}&offset=${offset}`)
  ])}catch(err){if(isNetError(err)){$('#page').innerHTML='<div class="panel"><h2>🌐 الطلبات الأونلاين</h2><div class="empty"><b>تعذر الاتصال بالإنترنت.</b><p>الطلبات المباشرة من الموقع لا تُعرض من بيانات ناقصة، ولم يتم تغيير أي طلب.</p></div></div>';return}throw err}
  ...
  const draw=()=>{
    ...
    const body=rows.map(entry=>{
      ...
      const address=w.order_type==='pickup'?'🏪 استلام من الفرع':`📍 ${esc(w.customer_address||w.delivery_address||'العنوان غير مسجل')}`;
...
async function renderDeliverySettings(){
 ...
 const [drivers,zones]=await Promise.all([rest('delivery_drivers',`select=*&branch_id=eq.${currentBranchId()}&order=active.desc,name`),rest('delivery_zones',`select=*&branch_id=eq.${currentBranchId()}&order=created_at.desc,name`)]);
 state.drivers=drivers||[];state.deliveryZones=zones||[];
 $('#page').innerHTML=`<div class="grid delivery-admin-grid"><div class="panel"><h2>🛵 المناديب</h2><div class="form-grid"><label>الاسم<input id="driverName"></label><label>الموبايل<input id="driverPhone" inputmode="tel"></label><label>الفرع<select id="driverBranch" disabled><option value="${currentBranchId()}">${esc(branchName(currentBranchId()))}</option></select></label></div><button id="addDriver" class="primary">إضافة مندوب</button><div class="manage-list">${drivers.filter(d=>d.active!==false).map(d=>`<div class="manage-row"><span>${esc(d.name)}${d.phone?` • ${esc(d.phone)}`:''}</span><span><button class="secondary" data-edit-driver="${d.id}">✏️</button><button class="danger" data-delete-driver="${d.id}">🗑️ حذف</button></span></div>`).join('')||'لا يوجد مناديب'}</div></div><div class="panel"><h2>📍 مناطق الدليفري</h2><div class="form-grid"><label>المنطقة<input id="zoneName"></label><label>الفرع المسؤول<select id="zoneBranch" disabled><option value="${currentBranchId()}">${esc(branchName(currentBranchId()))}</option></select></label><label>رسوم التوصيل<input id="zoneFee" type="number" min="0" step="0.01" value="0"></label></div><button id="addZone" class="primary">إضافة منطقة</button><div class="manage-list">${zones.map(z=>`<div class="manage-row ${z.active===false?'muted':''}"><span>${esc(z.name)} • ${money(z.delivery_fee)}</span><span><button class="secondary" data-edit-zone="${z.id}">✏️</button><button class="secondary" data-toggle-zone="${z.id}">${z.active===false?'تفعيل':'إيقاف'}</button></span></div>`).join('')||'لا توجد مناطق'}</div></div></div>`;
 $('#addDriver').onclick=async()=>{if(!$('#driverName').value.trim())return toast('اكتب اسم المندوب');const router=globalThis.__SharawlaPV2DeliverySettings;if(typeof router?.saveDriver!=='function')throw new Error('مسار إدارة المناديب غير جاهز');await router.saveDriver({name:$('#driverName').value.trim(),phone:$('#driverPhone').value.trim()||null,branch_id:currentBranchId(),active:true});toast('تمت إضافة المندوب');renderDeliverySettings()};
 $('#addZone').onclick=async()=>{const zoneNameValue=$('#zoneName').value.trim(),zoneFeeValue=Number($('#zoneFee')?.value||0);if(!zoneNameValue)return toast('اكتب اسم المنطقة');if(!Number.isFinite(zoneFeeValue)||zoneFeeValue<0)return toast('رسوم التوصيل غير صحيحة');const router=globalThis.__SharawlaPV2DeliverySettings;if(typeof router?.saveZone!=='function')throw new Error('مسار إدارة المناطق غير جاهز');await router.saveZone({name:zoneNameValue,delivery_fee:zoneFeeValue,branch_id:currentBranchId(),active:true});toast('تمت إضافة المنطقة');renderDeliverySettings()};
 $('#page').onclick=async e=>{const ed=e.target.closest('[data-edit-driver]');if(ed){const d=drivers.find(x=>String(x.id)===ed.dataset.editDriver);const name=await uiPrompt('اسم المندوب',d.name);if(name===null||!name.trim())return toast('اكتب اسم المندوب');const phone=await uiPrompt('الموبايل',d.phone||'');if(phone===null)return;const router=globalThis.__SharawlaPV2DeliverySettings;if(typeof router?.saveDriver!=='function')throw new Error('مسار إدارة المناديب غير جاهز');await router.saveDriver({id:d.id,branch_id:d.branch_id,name:name.trim(),phone:phone.trim()||null,active:d.active!==false});toast('تم تعديل المندوب');return renderDeliverySettings()}const td=e.target.closest('[data-delete-driver]');if(td){const d=drivers.find(x=>String(x.id)===td.dataset.deleteDriver);if(!d)return;if(!await uiConfirm(`حذف المندوب ${d.name}؟\nسيختفي من المناديب الحالية مع الاحتفاظ باسمه في الطلبات والتسويات القديمة.`))return;const router=globalThis.__SharawlaPV2DeliverySettings;if(typeof router?.saveDriver!=='function')throw new Error('مسار إدارة المناديب غير جاهز');await router.saveDriver({id:d.id,branch_id:d.branch_id,name:d.name,phone:d.phone||null,active:false});toast('تم حذف المندوب');return renderDeliverySettings()}const ez=e.target.closest('[data-edit-zone]');if(ez){const z=zones.find(x=>String(x.id)===ez.dataset.editZone);const name=await uiPrompt('اسم المنطقة',z.name);if(name===null||!name.trim())return toast('اكتب اسم المنطقة');const fee=await uiPrompt('رسوم التوصيل',String(Number(z.delivery_fee||0)),{type:'number'});if(fee===null||!Number.isFinite(Number(fee))||Number(fee)<0)return toast('رسوم التوصيل غير صحيحة');const router=globalThis.__SharawlaPV2DeliverySettings;if(typeof router?.saveZone!=='function')throw new Error('مسار إدارة المناطق غير جاهز');await router.saveZone({id:z.id,branch_id:z.branch_id,name:name.trim(),delivery_fee:Number(fee),active:z.active!==false});toast('تم تعديل المنطقة');return renderDeliverySettings()}const tz=e.target.closest('[data-toggle-zone]');if(tz){const z=zones.find(x=>String(x.id)===tz.dataset.toggleZone);if(!await uiConfirm(`${z.active===false?'تفعيل':'إيقاف'} المنطقة ${z.name}؟`))return;const router=globalThis.__SharawlaPV2DeliverySettings;if(typeof router?.saveZone!=='function')throw new Error('مسار إدارة المناطق غير جاهز');await router.saveZone({id:z.id,branch_id:z.branch_id,name:z.name,delivery_fee:Number(z.delivery_fee||0),active:z.active===false});toast('تم تحديث المنطقة');return renderDeliverySettings()}};
}
