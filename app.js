...
function offlineSaleReference(clientTx,n){
 const compact=String(clientTx||'').replace(/[^a-zA-Z0-9]/g,'').toUpperCase();
 return `OFF-${String(Math.max(1,Number(n)||1)).padStart(4,'0')}-${compact.slice(0,6)||'LOCAL'}`;
}
function legacyOfflineReference(o){
 const candidates=[o?.offline_reference,o?.bon_number,o?.invoice_number];
 return String(candidates.find(v=>/^OFF-/i.test(String(v||'')))||'').trim()||null;
}
function officialBonNumber(o){const n=Number(o?.bon_number);return Number.isFinite(n)&&n>0?n:null}
function officialInvoiceNumber(o){const n=Number(o?.invoice_number);return Number.isFinite(n)&&n>0?n:null}
function officialNumberPending(o){return !!(o?._official_number_pending||o?._offline||legacyOfflineReference(o))&&!officialBonNumber(o)}
function localOrderReference(o){return String(o?.offline_reference||legacyOfflineReference(o)||'').trim()||null}
...
function bonDisplay(o){const official=officialBonNumber(o);if(official)return String(official);const local=localOrderReference(o);return local||o?.order_number||'#'+o?.id}
async function updateNextBonBadge(){
 const el=$('#nextBonBadge');if(!el)return;
 try{
  const sh=await getOpenShift();
  if(!sh){el.innerHTML='<span>رقم البون التالي</span><b>—</b><small>افتح وردية</small>';return}
  if(!navigator.onLine||String(sh.id||'').startsWith('offline-shift-')){
   el.innerHTML='<span>رقم البون الرسمي</span><b>بعد المزامنة</b><small>كل بيع يأخذ مرجعًا محليًا مؤقتًا</small>';return;
  }
  const r=await rest('shift_bon_counters',`select=next_number&shift_id=eq.${sh.id}&limit=1`);
  let next=Number(r?.[0]?.next_number||0);
  if(!next){const oo=await rest('orders',`select=bon_number&shift_id=eq.${sh.id}&order=bon_number.desc&limit=1`);next=Number(oo?.[0]?.bon_number||0)+1}
  el.innerHTML=`<span>رقم البون التالي</span><b>${next||1}</b><small>وردية #${sh.id}</small>`;
 }catch(e){
  if(isNetError(e))el.innerHTML='<span>رقم البون الرسمي</span><b>بعد المزامنة</b><small>لا يتم تخمين رقم رسمي أثناء Offline</small>';
  else{console.error('next bon',e);el.innerHTML='<span>رقم البون التالي</span><b>—</b><small>تعذر قراءة العداد الرسمي</small>'}
 }
}
function invoiceDisplay(o){const official=officialInvoiceNumber(o);return official?String(official):'-';}
...
async function rc1PendingReturnUsage(orderId,baseAt=null){
 const used={},seen=new Set(),native=await rc1NativeOutboxRows(),nativeByTx=new Map(native.map(r=>[String(r.client_tx_id||''),r]));
 const add=(tx,pItems=[])=>{
  const key=String(tx||'');if(!key||seen.has(key))return;
  const n=nativeByTx.get(key);
  if(n&&n.operation_type==='return'&&!rc1ShouldOverlayOutbox(n,baseAt))return;
  for(const x of (pItems||[])){const itemKey=String(x.order_item_id??'');if(!itemKey)continue;used[itemKey]=(used[itemKey]||0)+Number(x.quantity||0)}
  seen.add(key);
 };
 let compat=[];try{compat=await offlineQueue()}catch{}
 for(const j of (compat||[])){
  if(j.type!=='return'||String(j.p_order_id)!==String(orderId))continue;
  add(j.client_tx_id,j.p_items||j.local_items||[]);
 }
 for(const n of native){
  if(n.operation_type!=='return')continue;
  const p=n.envelope?.payload?.rpc_payload||{};
  if(String(p.p_order_id)!==String(orderId))continue;
  add(n.client_tx_id,p.p_items||[]);
 }
 return used;
}
async function rc1ReturnUsageForOrder(o){
 let prior=[],baseAt=null,onlineRead=false;
 try{
  prior=await rest('return_items',`select=order_item_id,quantity,returns!inner(order_id)&returns.order_id=eq.${o.id}`);
  baseAt=new Date().toISOString();
  await odbSet(`returnUsage:${o.id}`,prior);
  await odbSet(`returnUsageAt:${o.id}`,baseAt);
  onlineRead=true;
 }catch(e){
  if(!isNetError(e))throw e;
  prior=(await odbGet(`returnUsage:${o.id}`))||[];
  baseAt=await odbGet(`returnUsageAt:${o.id}`);
 }
 const used={};for(const x of prior)used[String(x.order_item_id)]=(used[String(x.order_item_id)]||0)+Number(x.quantity||0);
 const pending=await rc1PendingReturnUsage(o.id,baseAt);
 for(const [k,v] of Object.entries(pending))used[k]=(used[k]||0)+Number(v||0);
 return {used,onlineRead,baseAt,pending};
}
function rc1AvailableReturnItems(items,used){
 return (items||[]).map(i=>({...i,_available:Math.max(0,Number(i.quantity||0)-Number(used[String(i.id)]||0))})).filter(i=>i._available>0);
}
async function openReturnForOrder(o,items){
 ...
 const usage=await rc1ReturnUsageForOrder(o);
 const used=usage.used;
 const available=rc1AvailableReturnItems(items,used);
...
function openDriverPicker(orderId, drivers, onDone){
 ...
 document.body.appendChild(m);
 m.onclick=async e=>{if(e.target===m||e.target.closest('[data-close]'))return m.remove();const b=e.target.closest('[data-driver-pick]');if(!b)return;const d=available.find(x=>String(x.id)===String(b.dataset.driverPick));if(!d)return;try{if(navigator.onLine===false)throw new Error('تسليم الطلب للمندوب يحتاج اتصال بالإنترنت حاليًا');const router=globalThis.__SharawlaPV2OrderDriverAssignment;if(typeof router?.assignDriver!=='function')throw new Error('مسار تسليم الطلب للمندوب غير جاهز');await router.assignDriver(orderId,d.id);await audit('assign_driver','order',orderId,{driver_id:d.id});m.remove();toast(`تم التسليم إلى ${d.name}`);if(onDone)onDone(d)}catch(err){toast(err.message)}};
}
