...
function openWebsiteAvailability(){
 if(!hasFeaturePermission('branchProductAvailability'))return toast('ليس لديك صلاحية إدارة توافر أصناف الموقع');
 const products=sortedActiveCategories().flatMap(c=>sortedActiveProducts(c.id));
 const m=document.createElement('div');m.className='modal';
 const rows=products.map(p=>`<tr><td><b>${esc(p.name)}</b></td>${state.branches.map(b=>{const bp=(state.branchProducts||[]).find(x=>String(x.product_id)===String(p.id)&&String(x.branch_id)===String(b.id));const on=bp?bp.active!==false:true;return `<td><button class="${on?'primary':'secondary'}" data-av-product="${p.id}" data-av-branch="${b.id}">${on?'✅ متاح':'❌ موقوف'}</button></td>`}).join('')}</tr>`).join('');
 m.innerHTML=`<div class="modal-card availability-modal"><div class="section-head"><div><h2>🌐 توافر أصناف الموقع</h2><p class="muted">إيقاف الصنف هنا يؤثر على الموقع في الفرع المحدد فقط، ولا يمنع بيعه من الكاشير.</p></div></div><div class="table-wrap"><table><thead><tr><th>الصنف</th>${state.branches.map(b=>`<th>${esc(b.name)}</th>`).join('')}</tr></thead><tbody>${rows}</tbody></table></div><div class="modal-actions"><button class="secondary" data-close>إغلاق</button></div></div>`;
 document.body.appendChild(m);
 m.onclick=async e=>{if(e.target===m||e.target.closest('[data-close]'))return m.remove();const bt=e.target.closest('[data-av-product]');if(!bt)return;const productId=Number(bt.dataset.avProduct),branchId=Number(bt.dataset.avBranch);const current=(state.branchProducts||[]).find(x=>Number(x.product_id)===productId&&Number(x.branch_id)===branchId);const next=current?current.active===false:false;bt.disabled=true;try{await rest('branch_products','',{method:'POST',headers:{Prefer:'resolution=merge-duplicates'},body:JSON.stringify([{branch_id:branchId,product_id:productId,active:next}])});state.branchProducts=await rest('branch_products','select=*');bt.className=next?'primary':'secondary';bt.textContent=next?'✅ متاح':'❌ موقوف';toast(next?'تم تشغيل الصنف على الموقع لهذا الفرع':'تم إيقاف الصنف على الموقع لهذا الفرع')}catch(err){toast(err.message||'تعذر تحديث التوافر')}finally{bt.disabled=false}};
}
