...
modifiers:[],productModifiers:[],productVariants:[],deliveryZones:[],drivers:[],branchPrintSettings:[],paymentMethods:[],branchPaymentMethods:[],branchFinancialSettings:[],websiteSettings:null,employeeBranches:[],userPermissions:null,selectedCustomer:null,activeBranchId:null,homeBranchId:null,customerAddresses:[]};
...
const ALL_PAGES=['home','pos','orders','returns','customers','deliveryOrders','deliverySettings','delivery','kitchen','shifts','inventory','expenses','products','branchProductAvailability','reports','users','settings'];
const ROLE_PAGES={
...
async function bootstrap(){
  const [emps,branches,cats,products,settingsRows,modifiers,productModifiers,productVariants,zones,drivers,printSettings,paymentMethods,branchPaymentMethods,branchFinancialSettings]=await Promise.all([
...
function renderPOS(){
 ...
 <aside class="cart">
  ...
  <div class="cart-foot">
   ...
   ${financialCfg().discount_enabled&&discountAllowed()?`<div class="totline discount-row"><span>خصم</span><div style="display:flex;gap:6px"><select id="discountType" style="width:82px"><option value="amount">مبلغ</option><option value="percent">%</option></select><input id="discount" type="number" min="0" value="0" style="width:90px;padding:5px"></div></div>`:''}<div class="totline" id="taxLine"><span>الضريبة</span><b id="taxAmount">0</b></div><div class="totline" id="serviceLine"><span>الخدمة</span><b id="serviceAmount">0</b></div>
...
function cartCalc(){const subtotal=state.cart.reduce((s,i)=>s+(Number(i.price||0)*Number(i.qty||0)),0);const f=financialCfg();let discountValue=Number($('#discount')?.value||0),discountType=$('#discountType')?.value||'amount';if(!f.discount_enabled||!discountAllowed())discountValue=0;if(discountType==='percent')discountValue=Math.min(Math.max(0,discountValue),Number(f.max_discount_percent||100));let discount=discountType==='percent'?subtotal*discountValue/100:Math.max(0,discountValue);discount=Math.min(subtotal,discount,subtotal*Number(f.max_discount_percent||100)/100);const base=Math.max(0,subtotal-discount);let taxAmount=f.tax_enabled?base*Number(f.tax_rate||0)/100:0;if(f.tax_enabled&&f.prices_include_tax)taxAmount=base-(base/(1+Number(f.tax_rate||0)/100));const serviceAmount=f.service_enabled?base*Number(f.service_rate||0)/100:0;let deliveryFee=0;if($('#orderType')?.value==='delivery')deliveryFee=Number($('#manualDeliveryFee')?.value||0);const total=Math.max(0,base+(f.tax_enabled&&!f.prices_include_tax?taxAmount:0)+serviceAmount+deliveryFee);return{subtotal,discount,discountType,discountValue,taxAmount,serviceAmount,deliveryFee,total}}
function drawCart(){if(!$('#cartItems'))return;$('#cartItems').innerHTML=state.cart.length?state.cart.map((i,n)=>`<div class="cart-item"><div class="cart-row"><b>${i.name}</b><b>${money(i.price*i.qty)}</b></div>${i.modifiers?.length?`<small>+ ${i.modifiers.map(x=>x.name).join('، ')}</small>`:''}${i.removed?.length?`<small>بدون: ${i.removed.join('، ')}</small>`:''}${i.notes?`<small>ملاحظة: ${i.notes}</small>`:''}<div class="qty"><button data-a="plus" data-i="${n}">+</button><b>${i.qty}</b><button data-a="minus" data-i="${n}">−</button><button data-a="del" data-i="${n}">🗑</button></div></div>`).join(''):'<div class="empty">أضف أصناف للأوردر</div>';$('#cartItems').onclick=e=>{const b=e.target.closest('button[data-a]');if(!b)return;const i=state.cart[+b.dataset.i];if(b.dataset.a==='plus')i.qty++;if(b.dataset.a==='minus'){i.qty--;if(i.qty<=0)state.cart.splice(+b.dataset.i,1)}if(b.dataset.a==='del')state.cart.splice(+b.dataset.i,1);drawCart()};const c=cartCalc();$('#subtotal').textContent=money(c.subtotal);if($('#deliveryFee'))$('#deliveryFee').textContent=money(c.deliveryFee);if($('#taxAmount'))$('#taxAmount').textContent=money(c.taxAmount);if($('#serviceAmount'))$('#serviceAmount').textContent=money(c.serviceAmount);if($('#taxLine'))$('#taxLine').classList.toggle('hidden',!financialCfg().tax_enabled);if($('#serviceLine'))$('#serviceLine').classList.toggle('hidden',!financialCfg().service_enabled);$('#grand').textContent=money(c.total)}
async function checkout(payment,payments=null){
 ...
 const order=[{
   ...
   payment_method:payment,subtotal:c.subtotal,discount:c.discount,discount_type:c.discountType,discount_value:c.discountValue,tax_amount:c.taxAmount,service_amount:c.serviceAmount,delivery_fee:c.deliveryFee,total:c.total,
 ...
 try{const payRows=(payments&&payments.length?payments:[{method:payment,amount:c.total}]).map(x=>({order_id:o.id,method:x.method,amount:Number(x.amount)}));await rest('order_payments','',{method:'POST',body:JSON.stringify(payRows)})}catch(e){}
 await audit('create_order','order',o.id,{invoice_number:o.invoice_number,bon_number:o.bon_number,total:c.total,payment,order_type:orderType,shift_id:openShift?.id||null});
 state.cart=[];state.selectedCustomer=null;document.getElementById('receiptPrintFrame')?.remove();toast(`تم حفظ بون ${bonDisplay(o)}`);renderPOS();showReceipt(o,savedItems);
}
...
const orderTypeLabel=v=>({takeaway:'استلام فرع',pickup:'استلام فرع',delivery:'دليفري',dinein:'صالة'}[v]||v||'');
...
function printCfg(branchId){return state.branchPrintSettings.find(x=>Number(x.branch_id)===Number(branchId))||{branch_id:Number(branchId),paper_size:'80',customer_copies:1,prep_copies:1,show_logo:true,show_business_name:true,show_branch_phone:true,show_branch_address:true,receipt_header:'',receipt_footer:'',prep_show_prices:false,auto_print_customer:false,auto_print_prep:false};}
...
function receiptHTML(o,items){const c=printCfg(o.branch_id);return `<div class="receipt">${receiptIdentityHTML(o,c)}<hr>${customerInfoHTML(o)}<div class="r-items">${items.map(i=>`<div><span>${esc(i.product_name)} × ${i.quantity}${i.notes?`<small>${esc(i.notes)}</small>`:''}</span><b>${money(i.total)}</b></div>`).join('')}</div><hr><div class="r-totals"><div><span>إجمالي الأصناف</span><b>${money(o.subtotal)}</b></div>${Number(o.discount||0)?`<div><span>خصم</span><b>-${money(o.discount)}</b></div>`:''}${Number(o.tax_amount||0)?`<div><span>الضريبة</span><b>${money(o.tax_amount)}</b></div>`:''}${Number(o.service_amount||0)?`<div><span>الخدمة</span><b>${money(o.service_amount)}</b></div>`:''}${Number(o.delivery_fee||0)?`<div><span>الدليفري</span><b>${money(o.delivery_fee)}</b></div>`:''}<div class="grand-print"><span>المطلوب</span><b>${money(o.total)}</b></div></div><hr><div class="r-footer">${orderTypeLabel(o.order_type)} • ${paymentLabel(o.payment_method)}<br>${esc(c.receipt_footer||state.business?.receipt_footer||'شكرًا لزيارتكم')}</div></div>`}
function printIsolated(html,paper='80',copies=1){const old=document.getElementById('receiptPrintFrame');if(old)old.remove();const f=document.createElement('iframe');f.id='receiptPrintFrame';f.setAttribute('aria-hidden','true');f.style.cssText='position:fixed;width:1px;height:1px;right:-9999px;bottom:-9999px;border:0;opacity:0;pointer-events:none';document.body.appendChild(f);const width=paper==='58'?'54mm':'76mm',page=paper==='58'?'58mm':'80mm';const n=Math.max(1,Math.min(5,Number(copies)||1));const body=Array.from({length:n},(_,i)=>`<div class="print-copy">${html}</div>`).join('');const d=f.contentDocument;d.open();d.write(`<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><style>@page{size:${page} auto;margin:2mm}body{margin:0;font-family:Arial,sans-serif;color:#000;direction:rtl}.print-copy{width:${width}}.print-copy:not(:last-child){page-break-after:always}.receipt{width:${width};font-size:12px;line-height:1.45}.receipt-logo{display:block;max-width:32mm;max-height:22mm;object-fit:contain;margin:2mm auto}.receipt h2{text-align:center;font-size:19px;margin:3px 0}.receipt h1{text-align:center}.receipt hr{border:0;border-top:1px dashed #000;margin:6px 0}.r-meta,.r-footer{text-align:center}.r-customer{line-height:1.7}.r-items>div,.r-totals>div{display:flex;justify-content:space-between;gap:8px;margin:5px 0}.r-items small{display:block}.grand-print{font-size:15px;font-weight:900;border-top:1px dashed #000;padding-top:6px}.shift-print{width:${width};font-size:11px}.shift-print h2,.shift-print h3{text-align:center;margin:5px 0}.shift-report-table{width:100%;border-collapse:collapse;font-size:10px}.shift-report-table th,.shift-report-table td{border-bottom:1px dashed #777;padding:4px 2px;text-align:right}.shift-report-table th:nth-child(n+2),.shift-report-table td:nth-child(n+2){text-align:left}.shift-report-table small{display:block;font-size:8px}</style></head><body>${body}</body></html>`);d.close();setTimeout(()=>{try{f.contentWindow.focus();f.contentWindow.print()}finally{setTimeout(()=>f.remove(),1200)}},150)}
function printReceipt(o,items){const c=printCfg(o.branch_id);printIsolated(receiptHTML(o,items),c.paper_size,c.customer_copies)}
function prepReceiptHTML(o,items){const c=printCfg(o.branch_id);return `<div class="receipt"><div class="receipt-head"><h2>ريسيت التحضير</h2><h1>بون ${esc(bonDisplay(o))}</h1><p>${branchName(o.branch_id)} • ${fmtDate(o.created_at)}</p><p>${orderTypeLabel(o.order_type)}</p></div><hr>${customerInfoHTML(o)}<div class="r-items">${items.map(i=>`<div class="prep-item"><span><b>${i.quantity} × ${esc(i.product_name)}</b>${i.notes?`<small class="prep-note">📝 ${esc(i.notes)}</small>`:''}</span>${c.prep_show_prices?`<b>${money(i.total)}</b>`:''}</div>`).join('')}</div>${c.prep_show_prices?`<hr><div class="r-totals"><div><span>إجمالي الأصناف</span><b>${money(o.subtotal)}</b></div></div>`:''}<div class="prep-service-note">${c.prep_show_prices?'بدون رسوم خدمة/دليفري':''}</div></div>`}
function printPrepReceipt(o,items){const c=printCfg(o.branch_id);printIsolated(prepReceiptHTML(o,items),c.paper_size,c.prep_copies)}
function autoPrintOrder(o,items){const c=printCfg(o.branch_id);if(c.auto_print_prep&&state.settings.enable_prep_receipt)setTimeout(()=>printPrepReceipt(o,items),250);if(c.auto_print_customer&&state.settings.enable_receipt_print)setTimeout(()=>printReceipt(o,items),900)}
...
async function openReturnForOrder(o,items){
 ...
 let prior=[];try{prior=await rest('return_items',`select=order_item_id,quantity,returns!inner(order_id)&returns.order_id=eq.${o.id}`)}catch(e){prior=[]}
...
async function shiftMetrics(shift){
 ...
 return{orders,valid,sales,grossSales,returnTotal,returns,cash,wallet,instapay,returnCash,returnWallet,returnInstapay,exp,expected,count:valid.length,cancelled:orders.filter(o=>o.status==='cancelled').length}
}
...
async function renderSettings(){
 ...
 if(canPrinting){const pc=printCfg(currentBranchId());printingPanel=`<div class="panel"><h2>🖨️ إعدادات الطباعة — ${esc(branchName(currentBranchId()))}</h2><p>الإعدادات دي خاصة بالفرع الحالي، وبيانات الهاتف والعنوان بتتسحب من بيانات الفرع.</p><div class="form-grid"><label>مقاس الورق<select id="prPaper"><option value="80" ${pc.paper_size==='80'?'selected':''}>80 مم</option><option value="58" ${pc.paper_size==='58'?'selected':''}>58 مم</option></select></label><label>عدد نسخ فاتورة العميل<input id="prCustomerCopies" type="number" min="1" max="5" value="${Number(pc.customer_copies||1)}"></label><label>عدد نسخ التحضير<input id="prPrepCopies" type="number" min="1" max="5" value="${Number(pc.prep_copies||1)}"></label><label>نص أعلى الفاتورة<input id="prHeader" value="${esc(pc.receipt_header||'')}"></label><label class="wide">نص أسفل الفاتورة لهذا الفرع<input id="prFooter" value="${esc(pc.receipt_footer||'')}" placeholder="لو فاضي يستخدم النص العام للنشاط"></label></div><div class="settings-list">${[['prLogo','إظهار اللوجو',pc.show_logo],['prName','إظهار اسم النشاط',pc.show_business_name],['prPhone','إظهار هاتف الفرع',pc.show_branch_phone],['prAddress','إظهار عنوان الفرع',pc.show_branch_address],['prPrepPrices','إظهار الأسعار في ريسيت التحضير',pc.prep_show_prices],['prAutoCustomer','طباعة فاتورة العميل تلقائيًا بعد البيع/استلام طلب الموقع',pc.auto_print_customer],['prAutoPrep','طباعة ريسيت التحضير تلقائيًا بعد البيع/استلام طلب الموقع',pc.auto_print_prep]].map(([id,l,v])=>`<label class="setting-switch"><span>${l}</span><input id="${id}" type="checkbox" ${v?'checked':''}></label>`).join('')}</div><button id="savePrintSettings" class="primary">حفظ إعدادات طباعة الفرع</button></div>`;}
 ...
 if($('#savePrintSettings'))$('#savePrintSettings').onclick=async()=>{try{const row={branch_id:currentBranchId(),paper_size:$('#prPaper').value,customer_copies:Math.max(1,Math.min(5,Number($('#prCustomerCopies').value||1))),prep_copies:Math.max(1,Math.min(5,Number($('#prPrepCopies').value||1))),show_logo:$('#prLogo').checked,show_business_name:$('#prName').checked,show_branch_phone:$('#prPhone').checked,show_branch_address:$('#prAddress').checked,receipt_header:$('#prHeader').value.trim()||null,receipt_footer:$('#prFooter').value.trim()||null,prep_show_prices:$('#prPrepPrices').checked,auto_print_customer:$('#prAutoCustomer').checked,auto_print_prep:$('#prAutoPrep').checked,updated_at:new Date().toISOString()};await rest('branch_print_settings','',{method:'POST',headers:{Prefer:'resolution=merge-duplicates'},body:JSON.stringify([row])});state.branchPrintSettings=state.branchPrintSettings.filter(x=>Number(x.branch_id)!==currentBranchId()).concat(row);toast('تم حفظ إعدادات طباعة الفرع')}catch(e){toast(e.message)}};
