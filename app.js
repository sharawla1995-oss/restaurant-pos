...
async function renderProducts(){
 ...
 $('#page').innerHTML=`<div class="panel"><h2>التصنيفات</h2><div class="toolbar"><input id="newCategory" placeholder="اسم التصنيف"><button id="addCategory" class="primary">إضافة تصنيف</button></div><div class="chips">${state.categories.filter(c=>c.active!==false).map(c=>`<span class="chip">${esc(c.name)} <button class="secondary" data-edit-cat="${c.id}">✏️</button> <button class="danger" data-delete-cat="${c.id}">🗑️ حذف</button></span>`).join('')}</div></div><div class="panel"><h2>إضافة صنف</h2><div class="form-grid"><label>الاسم<input id="pName"></label><label>القسم<select id="pCat">${state.categories.filter(c=>c.active!==false).map(c=>`<option value="${c.id}">${c.name}</option>`).join('')}</select></label><label>سعر البيع<input id="pPrice" type="number"></label><label>التكلفة<input id="pCost" type="number"></label><label>باركود<input id="pBarcode"></label><label>مكونات يمكن حذفها<input id="pRemovals" placeholder="بصل، مخلل، صوص"></label></div><div class="toggle-row"><label><input id="pExtras" type="checkbox" checked> يسمح بإضافات</label><label><input id="pRemove" type="checkbox" checked> يسمح بحذف مكونات</label><label><input id="pNotes" type="checkbox" checked> يسمح بملاحظة</label></div><button id="addProduct" class="primary">إضافة الصنف</button></div>
