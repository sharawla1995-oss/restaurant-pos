...
function renderPOS(){
 ...
 <aside class="cart">
  ...
  <div id="deliveryFields" class="delivery-fields hidden">
   ...
   <div class="delivery-fee-control">
    <select id="deliveryFeeMode"><option value="auto">رسوم التوصيل — تلقائي</option><option value="manual">رسوم التوصيل — يدوي</option></select>
    <input id="manualDeliveryFee" class="hidden" type="number" min="0" step="0.01" inputmode="decimal" placeholder="اكتب رسوم التوصيل">
   </div>
 ...
 $('#deliveryFeeMode')?.addEventListener('change',()=>{const manual=$('#deliveryFeeMode').value==='manual';$('#manualDeliveryFee')?.classList.toggle('hidden',!manual);if(manual){const autoFee=Number($('#deliveryZone')?.selectedOptions?.[0]?.dataset?.fee||0);if(!$('#manualDeliveryFee').value)$('#manualDeliveryFee').value=autoFee||'';}drawCart();});
...
async function lookupCustomerByPhone(){
 ...
 try{
   ...
   if(rows?.length){
     ...
     if($('#deliveryFeeMode'))$('#deliveryFeeMode').value='auto';
...
function cartCalc(){const subtotal=state.cart.reduce((s,i)=>s+(i.price*i.qty),0);const discount=Number($('#discount')?.value||0);let deliveryFee=0;if($('#orderType')?.value==='delivery'){const manual=$('#deliveryFeeMode')?.value==='manual';deliveryFee=manual?Number($('#manualDeliveryFee')?.value||0):Number($('#deliveryZone')?.selectedOptions?.[0]?.dataset?.fee||0)}return{subtotal,discount,deliveryFee,total:Math.max(0,subtotal-discount+deliveryFee)}}
...
async function checkout(payment){
 ...
 if(orderType==='delivery'&&!$('#deliveryZone')?.value)return toast('اختر منطقة الدليفري أولاً');
...
async function renderDelivery(){
 ...
 $('#addZone').onclick=async()=>{if(!$('#zoneName').value.trim())return toast('اكتب اسم المنطقة');await rest('delivery_zones','',{method:'POST',body:JSON.stringify([{name:$('#zoneName').value.trim(),delivery_fee:Number($('#zoneFee').value||0),branch_id:currentBranchId(),active:true}])});toast('تمت إضافة المنطقة');renderDelivery()};
...
async function renderExpenses(){const rows=await rest('expenses',`select=*&branch_id=eq.${currentBranchId()}&order=created_at.desc&limit=200`);$('#page').innerHTML=`<div class="panel"><h2>إضافة مصروف</h2><div class="form-grid"><label>البيان<input id="exTitle"></label><label>المبلغ<input id="exAmount" type="number" min="0" step="0.01"></label></div><button id="addExpense" class="primary">حفظ المصروف</button></div><div class="panel"><div class="table-wrap"><table><thead><tr><th>التاريخ</th><th>البيان</th><th>المبلغ</th><th>الفرع</th><th>الوردية</th></tr></thead><tbody>${rows.map(e=>`<tr><td>${fmtDate(e.created_at)}</td><td>${esc(e.description)}</td><td>${money(e.amount)}</td><td>${branchName(e.branch_id)}</td><td>${e.shift_id?'#'+e.shift_id:'-'}</td></tr>`).join('')}</tbody></table></div></div>`;$('#addExpense').onclick=async()=>{if(!$('#exTitle').value||!$('#exAmount').value)return toast('أدخل البيان والمبلغ');const shift=await getOpenShift();if(!shift)return toast('افتح وردية أولًا قبل تسجيل المصروف');const created=await rest('expenses','select=*',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify([{branch_id:currentBranchId(),employee_id:state.employee.id,shift_id:shift.id,description:$('#exTitle').value,amount:Number($('#exAmount').value)}])});await audit('create_expense','expense',created?.[0]?.id,{amount:Number($('#exAmount').value),shift_id:shift.id});toast('تم حفظ المصروف وربطه بالوردية');renderExpenses()}}
...
async function renderDeliverySettings(){
 ...
 $('#addZone').onclick=async()=>{if(!$('#zoneName').value.trim())return toast('اكتب اسم المنطقة');await rest('delivery_zones','',{method:'POST',body:JSON.stringify([{name:$('#zoneName').value.trim(),delivery_fee:Number($('#zoneFee').value||0),branch_id:currentBranchId(),active:true}])});toast('تمت إضافة المنطقة');renderDeliverySettings()};
