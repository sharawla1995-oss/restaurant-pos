...
async function saveOfflineExpense(shift,description,amount){const q=await offlineQueue(),clientTx=uuid(),created=new Date().toISOString();const local={id:`offline-exp-${clientTx}`,branch_id:currentBranchId(),employee_id:state.employee.id,shift_id:shift.id,description,amount,created_at:created,_offline:true};q.push({type:'expense',client_tx_id:clientTx,created_at:created,p_shift_id:Number(shift.id),p_description:description,p_amount:Number(amount),local_expense:local});await setOfflineQueue(q);return local}
async function saveOfflineReturn(o,selected,reason,notes,method,total,available){const q=await offlineQueue(),clientTx=uuid(),created=new Date().toISOString(),sh=await getOpenShift();const localReturn={id:`offline-ret-${clientTx}`,return_number:`OFF-${clientTx.slice(0,6)}`,branch_id:o.branch_id,order_id:o.id,shift_id:sh?.id,original_invoice_number:o.invoice_number,original_bon_number:o.bon_number,reason,notes,subtotal:total,total,created_at:created,_offline:true};const localItems=selected.map(x=>{const i=available.find(z=>String(z.id)===String(x.order_item_id));const unit=Number(i?.total||0)/Number(i?.quantity||1);return {return_id:localReturn.id,order_item_id:x.order_item_id,product_name:i?.product_name||'صنف',quantity:x.quantity,unit_refund:unit,total:unit*x.quantity}});const localPayments=[{return_id:localReturn.id,method,amount:total}];q.push({type:'return',client_tx_id:clientTx,created_at:created,p_order_id:Number(o.id),p_reason:reason,p_notes:notes,p_items:selected,p_payments:[{method,amount:total}],local_return:localReturn,local_items:localItems,local_payments:localPayments});await setOfflineQueue(q);return {r:localReturn,items:localItems,payments:localPayments}}
async function syncOfflineQueue(){if(!navigator.onLine||!session?.access_token)return;let q=await offlineQueue();if(!q.length)return;let done=0;for(const job of [...q]){try{if(job.type==='sale')await rpc('create_pos_order_atomic',{p_order:job.p_order,p_items:job.p_items,p_payments:job.p_payments});else if(job.type==='expense')await rpc('create_pos_expense_idempotent',{p_shift_id:job.p_shift_id,p_description:job.p_description,p_amount:job.p_amount,p_client_tx_id:job.client_tx_id});else if(job.type==='return')await rpc('create_order_return_idempotent',{p_order_id:job.p_order_id,p_reason:job.p_reason,p_notes:job.p_notes,p_items:job.p_items,p_payments:job.p_payments,p_client_tx_id:job.client_tx_id});q=q.filter(x=>x.client_tx_id!==job.client_tx_id);await setOfflineQueue(q);done++}catch(e){console.warn('sync stopped',e);break}}if(done)toast(`تمت مزامنة ${done} حركة أوفلاين`)}
