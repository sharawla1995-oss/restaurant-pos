...
modifiers:[],productModifiers:[],deliveryZones:[],drivers:[],employeeBranches:[],userPermissions:null,selectedCustomer:null,activeBranchId:null,homeBranchId:null,customerAddresses:[]};
...
const ALL_PAGES=['home','pos','orders','customers','deliveryOrders','deliverySettings','delivery','kitchen','shifts','inventory','expenses','products','reports','users','settings'];
const ROLE_PAGES={
...
async function bootstrap(){
  ...
  state.employee=emps[0];state.homeBranchId=Number(emps[0].branch_id||0);state.branches=branches||[];state.categories=cats||[];state.products=products||[];state.modifiers=modifiers||[];state.productModifiers=productModifiers||[];state.deliveryZones=zones||[];state.drivers=drivers||[];try{state.employeeBranches=await rest('employee_branches',`select=branch_id&employee_id=eq.${state.employee.id}`)}catch(e){state.employeeBranches=[]}try{state.userPermissions=await rest('employee_permissions',`select=permission_key,allowed&employee_id=eq.${state.employee.id}`)}catch(e){state.userPermissions=null}for(const r of (settingsRows||[])){if(r.key in state.settings)state.settings[r.key]=String(r.value)==='true';}
...
async function renderProducts(){
 ...
 <div class="panel"><h2>الأصناف</h2><div class="table-wrap"><table><thead><tr><th>الصورة</th><th>الصنف</th><th>السعر</th><th>الإضافات المتاحة</th><th>إجراء</th></tr></thead><tbody>${state.products.map(p=>`<tr><td>${p.image_url?`<img class="admin-product-img" src="${esc(p.image_url)}">`:'🍔'}</td><td>${p.name}</td><td>${money(p.price)}</td><td>${productModifierList(p).map(x=>x.name).join('، ')||'-'}</td><td><button class="secondary" data-image-product="${p.id}">🖼️ صورة</button> <button class="secondary" data-edit-product="${p.id}">✏️ تعديل</button> <button class="secondary" data-config="${p.id}">خيارات</button> <button class="secondary" data-toggle-product="${p.id}">${p.active===false?'تفعيل':'إيقاف'}</button></td></tr>`).join('')}</tbody></table></div></div>`;
 ...
 $('#addCategory').onclick=async()=>{const name=$('#newCategory').value.trim();if(!name)return toast('اكتب اسم التصنيف');await rest('categories','',{method:'POST',body:JSON.stringify([{name,active:true}])});toast('تمت إضافة التصنيف');await reloadCatalog();renderProducts()};
 $('#page').onclick=async e=>{const ip=e.target.closest('[data-image-product]');if(ip){const p=state.products.find(x=>String(x.id)===ip.dataset.imageProduct);if(p)return openProductImage(p)}const cfg=e.target.closest('[data-config]');if(cfg)return openProductConfig(state.products.find(p=>String(p.id)===String(cfg.dataset.config)));const ep=e.target.closest('[data-edit-product]');if(ep){const p=state.products.find(x=>String(x.id)===ep.dataset.editProduct);const name=prompt('اسم الصنف',p.name);if(name===null)return;const price=prompt('سعر البيع',p.price);if(price===null||!Number.isFinite(Number(price)))return;const cost=prompt('التكلفة',p.cost||0);if(cost===null||!Number.isFinite(Number(cost)))return;await rest('products',`id=eq.${p.id}`,{method:'PATCH',body:JSON.stringify({name:name.trim(),price:Number(price),cost:Number(cost)})});toast('تم تعديل الصنف');return renderProducts()}const tp=e.target.closest('[data-toggle-product]');if(tp){const p=state.products.find(x=>String(x.id)===tp.dataset.toggleProduct);if(!confirm(`${p.active===false?'تفعيل':'إيقاف'} الصنف ${p.name}؟`))return;await rest('products',`id=eq.${p.id}`,{method:'PATCH',body:JSON.stringify({active:p.active===false})});toast('تم تحديث الصنف');return renderProducts()}const ec=e.target.closest('[data-edit-cat]');if(ec){const c=state.categories.find(x=>String(x.id)===ec.dataset.editCat);const name=prompt('اسم التصنيف',c.name);if(name===null)return;await rest('categories',`id=eq.${c.id}`,{method:'PATCH',body:JSON.stringify({name:name.trim()})});await reloadCatalog();return renderProducts()}const tc=e.target.closest('[data-toggle-cat]');if(tc){const c=state.categories.find(x=>String(x.id)===tc.dataset.toggleCat);if(!confirm(`${c.active===false?'تفعيل':'إيقاف'} التصنيف ${c.name}؟`))return;await rest('categories',`id=eq.${c.id}`,{method:'PATCH',body:JSON.stringify({active:c.active===false})});await reloadCatalog();return renderProducts()}};
}
...
async function resetGroups(groups){const order=['order_item_modifiers','order_payments','order_items','driver_settlements','expenses','orders','shifts','customer_addresses','customers','product_modifiers','branch_products','modifiers','products','categories','delivery_drivers','delivery_zones','employee_permissions','employee_branches','app_settings'];const wanted=new Set(groups.flatMap(g=>BACKUP_GROUPS[g].tables));for(const t of order){if(!wanted.has(t))continue;await rest(t,'id=not.is.null',{method:'DELETE'}).catch(async()=>{await rest(t,'',{method:'DELETE'})})}}
...
async function renderSettings(){
 ...
 const groups=Object.entries(BACKUP_GROUPS).map(([k,v])=>`<label class="backup-check"><input type="checkbox" data-backup-group="${k}" checked> ${v.label}</label>`).join('');
 ...
 $('#restoreBackup').onclick=async()=>{const f=$('#restoreFile').files[0],g=selectedBackupGroups($('#page'));if(!f)return toast('اختر ملف النسخة');if(!g.length)return toast('حدد ما تريد استعادته');if(!confirm('استعادة البيانات المحددة من النسخة؟ سيتم دمجها مع البيانات الحالية.'))return;try{await restoreBackup(f,g);toast('تمت الاستعادة بنجاح')}catch(e){toast(e.message)}};
 $('#resetSelected').onclick=async()=>{const g=selectedBackupGroups($('#page'));if(!g.length)return toast('حدد ما تريد إعادة ضبطه');const code=prompt('عملية خطيرة. اكتب RESET للتأكيد');if(code!=='RESET')return toast('تم إلغاء إعادة الضبط');try{await resetGroups(g);toast('تمت إعادة ضبط البيانات المحددة')}catch(e){toast(e.message)}};
}
...
async function renderDeliverySettings(){
 ...
 $('#page').onclick=async e=>{const ed=e.target.closest('[data-edit-driver]');if(ed){const d=drivers.find(x=>String(x.id)===ed.dataset.editDriver);const name=prompt('اسم المندوب',d.name);if(name===null)return;const phone=prompt('الموبايل',d.phone||'');if(phone===null)return;await rest('delivery_drivers',`id=eq.${d.id}`,{method:'PATCH',body:JSON.stringify({name:name.trim(),phone:phone.trim()||null})});toast('تم تعديل المندوب');return renderDeliverySettings()}const td=e.target.closest('[data-toggle-driver]');if(td){const d=drivers.find(x=>String(x.id)===td.dataset.toggleDriver);if(!confirm(`${d.active===false?'تفعيل':'إيقاف'} المندوب ${d.name}؟`))return;await rest('delivery_drivers',`id=eq.${d.id}`,{method:'PATCH',body:JSON.stringify({active:d.active===false})});toast('تم تحديث المندوب');return renderDeliverySettings()}const ez=e.target.closest('[data-edit-zone]');if(ez){const z=zones.find(x=>String(x.id)===ez.dataset.editZone);const name=prompt('اسم المنطقة',z.name);if(name===null)return;await rest('delivery_zones',`id=eq.${z.id}`,{method:'PATCH',body:JSON.stringify({name:name.trim()})});toast('تم تعديل المنطقة');return renderDeliverySettings()}const tz=e.target.closest('[data-toggle-zone]');if(tz){const z=zones.find(x=>String(x.id)===tz.dataset.toggleZone);if(!confirm(`${z.active===false?'تفعيل':'إيقاف'} المنطقة ${z.name}؟`))return;await rest('delivery_zones',`id=eq.${z.id}`,{method:'PATCH',body:JSON.stringify({active:z.active===false})});toast('تم تحديث المنطقة');return renderDeliverySettings()}const b=e.target.closest('[data-settle]');if(!b)return;const did=Number(b.dataset.settle);const d=drivers.find(x=>x.id===did);const os=await rest('orders',`select=id,total&driver_id=eq.${did}&status=eq.delivered&payment_method=eq.cash&driver_settled_at=is.null`);const amount=os.reduce((a,o)=>a+Number(o.total||0),0);if(!os.length)return toast('لا توجد تحصيلات كاش غير مسواة لهذا المندوب');if(!confirm(`تسوية ${d.name}: ${os.length} طلب بإجمالي ${money(amount)}؟`))return;const now=new Date().toISOString();await rest('orders',`id=in.(${os.map(o=>o.id).join(',')})`,{method:'PATCH',body:JSON.stringify({driver_settled_at:now})});await rest('driver_settlements','',{method:'POST',body:JSON.stringify([{driver_id:did,branch_id:d.branch_id,employee_id:state.employee.id,orders_count:os.length,amount,created_at:now}])});await audit('driver_settlement','delivery_driver',did,{orders_count:os.length,amount});toast('تمت تسوية المندوب');renderDeliverySettings()};
}
