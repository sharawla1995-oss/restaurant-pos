...
modifiers:[],productModifiers:[],productVariants:[],deliveryZones:[],drivers:[],branchPrintSettings:[],paymentMethods:[],branchPaymentMethods:[],branchFinancialSettings:[],websiteSettings:null,employeeBranches:[],userPermissions:null,selectedCustomer:null,activeBranchId:null,homeBranchId:null,customerAddresses:[],activePromo:null};
...
async function checkout(payment,payments=null){
 ...
 if(state.activePromo?.id&&c.promoDiscount>0){try{await rpc('redeem_promo_code',{p_promo_id:Number(state.activePromo.id),p_code:state.activePromo.code,p_branch_id:branchId,p_channel:'pos',p_customer_phone:phone||null,p_discount:c.promoDiscount,p_order_id:Number(o.id),p_website_order_id:null})}catch(e){console.error('promo redemption',e)}}
...
function receiptHTML(o,items){const c=printCfg(o.branch_id);return `<div class="receipt">${receiptIdentityHTML(o,c)}<hr>${customerInfoHTML(o)}<div class="r-items">${items.map(i=>`<div><span>${esc(i.product_name)} × ${i.quantity}${i.notes?`<small>${esc(i.notes)}</small>`:''}</span><b>${money(i.total)}</b></div>`).join('')}</div><hr><div class="r-totals"><div><span>إجمالي الأصناف</span><b>${money(o.subtotal)}</b></div>${Number(o.discount||0)?`<div><span>${o.promo_code?'خصم ('+esc(o.promo_code)+')':'خصم'}</span><b>-${money(o.discount)}</b></div>`:''}${Number(o.tax_amount||0)?`<div><span>الضريبة</span><b>${money(o.tax_amount)}</b></div>`:''}${Number(o.service_amount||0)?`<div><span>الخدمة</span><b>${money(o.service_amount)}</b></div>`:''}${Number(o.delivery_fee||0)?`<div><span>الدليفري</span><b>${money(o.delivery_fee)}</b></div>`:''}<div class="grand-print"><span>المطلوب</span><b>${money(o.total)}</b></div></div><hr><div class="r-footer">${orderTypeLabel(o.order_type)} • ${paymentLabel(o.payment_method)}<br>${esc(c.receipt_footer||state.business?.receipt_footer||'شكرًا لزيارتكم')}</div></div>`}
