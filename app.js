...
async function renderProducts(){
 ...
 $('#page').onclick=async e=>{const ip=e.target.closest('[data-image-product]');if(ip){const p=state.products.find(x=>String(x.id)===ip.dataset.imageProduct);if(p)return openProductImage(p)}const cfg=e.target.closest('[data-config]');if(cfg)return openProductConfig(state.products.find(p=>String(p.id)===String(cfg.dataset.config)));const ep=e.target.closest('[data-edit-product]');if(ep){const p=state.products.find(x=>String(x.id)===ep.dataset.editProduct);const name=await uiPrompt('اسم الصنف',p.name);if(name===null)return;const price=await uiPrompt('سعر البيع',p.price);if(price===null||!Number.isFinite(Number(price)))return;const cost=await uiPrompt('التكلفة',p.cost||0);if(cost===null||!Number.isFinite(Number(cost)))return;await rest('products',`id=eq.${p.id}`,{method:'PATCH',body:JSON.stringify({name:name.trim(),price:Number(price),cost:Number(cost)})});toast('تم تعديل الصنف');return renderProducts()}const tp=e.target.closest('[data-toggle-product]');if(tp){const p=state.products.find(x=>String(x.id)===tp.dataset.toggleProduct);if(!await uiConfirm(`${p.active===false?'تفعيل':'إيقاف'} الصنف ${p.name}؟`))return;await rest('products',`id=eq.${p.id}`,{method:'PATCH',body:JSON.stringify({active:p.active===false})});toast('تم تحديث الصنف');return renderProducts()}const ec=e.target.closest('[data-edit-cat]');if(ec){const c=state.categories.find(x=>String(x.id)===ec.dataset.editCat);const name=await uiPrompt('اسم التصنيف',c.name);if(name===null)return;await rest('categories',`id=eq.${c.id}`,{method:'PATCH',body:JSON.stringify({name:name.trim()})});await reloadCatalog();return renderProducts()}const tc=e.target.closest('[data-toggle-cat]');if(tc){const c=state.categories.find(x=>String(x.id)===tc.dataset.toggleCat);if(!await uiConfirm(`${c.active===false?'تفعيل':'إيقاف'} التصنيف ${c.name}؟`))return;await rest('categories',`id=eq.${c.id}`,{method:'PATCH',body:JSON.stringify({active:c.active===false})});await reloadCatalog();return renderProducts()}};
}
...
async function renderSettings(){
 ...
 $('#resetSelected').onclick=async()=>{const g=selectedBackupGroups($('#page'));if(!g.length)return toast('حدد ما تريد إعادة ضبطه');const code=await uiPrompt('عملية خطيرة. اكتب RESET للتأكيد');if(code!=='RESET')return toast('تم إلغاء إعادة الضبط');try{await resetGroups(g);toast('تمت إعادة ضبط البيانات المحددة')}catch(e){toast(e.message)}};
}
