...
async function renderSettings(){
 ...
 if($('#savePrintSettings'))$('#savePrintSettings').onclick=async()=>{try{if(window.topBurgerDesktop?.print){try{localStorage.setItem(desktopPrinterKey(currentBranchId(),'customer'),$('#prCustomerPrinter')?.value||'');localStorage.setItem(desktopPrinterKey(currentBranchId(),'prep'),$('#prPrepPrinter')?.value||'')}catch{}}const row={branch_id:currentBranchId(),paper_size:$('#prPaper').value,customer_copies:Math.max(1,Math.min(5,Number($('#prCustomerCopies').value||1))),prep_copies:Math.max(1,Math.min(5,Number($('#prPrepCopies').value||1))),show_logo:$('#prLogo').checked,show_business_name:$('#prName').checked,show_branch_phone:$('#prPhone').checked,show_branch_address:$('#prAddress').checked,receipt_header:$('#prHeader').value.trim()||null,receipt_footer:$('#prFooter').value.trim()||null,prep_show_prices:true,auto_print_customer:$('#prAutoCustomer').checked,auto_print_prep:$('#prAutoPrep').checked,updated_at:new Date().toISOString()};await rest('branch_print_settings','',{method:'POST',headers:{Prefer:'resolution=merge-duplicates'},body:JSON.stringify([row])});state.branchPrintSettings=state.branchPrintSettings.filter(x=>Number(x.branch_id)!==currentBranchId()).concat(row);toast('تم حفظ إعدادات طباعة الفرع')}catch(e){toast(e.message)}};
