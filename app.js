...
async function getOpenShift(employeeId=state.employee.id,branchId=state.employee.branch_id){const rows=await rest('shifts',`select=*&employee_id=eq.${employeeId}&branch_id=eq.${branchId}&status=eq.open&closed_at=is.null&order=opened_at.desc&limit=1`);return rows?.[0]||null}
async function audit(action,entityType,entityId,details={}){try{await rest('audit_logs','',{method:'POST',body:JSON.stringify([{employee_id:state.employee.id,branch_id:state.employee.branch_id,action,entity_type:entityType,entity_id:entityId||null,details}])})}catch(e){}}
...
function refreshDeliveryDrivers(){const el=$('#deliveryDriver');if(!el)return;const branch=$('#deliveryBranch')?.value;const current=el.value;const list=state.drivers.filter(d=>d.active!==false&&(!branch||String(d.branch_id)===String(branch)));el.innerHTML='<option value="">المندوب — يحدد لاحقًا</option>'+list.map(d=>`<option value="${d.id}">${d.name}${d.phone?` — ${d.phone}`:''}</option>`).join('');if(list.some(d=>String(d.id)===String(current)))el.value=current;}
