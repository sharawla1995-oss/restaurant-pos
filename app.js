...
function printIsolated(html,paper='80',copies=1){const old=document.getElementById('receiptPrintFrame');if(old)old.remove();const f=document.createElement('iframe');f.id='receiptPrintFrame';f.setAttribute('aria-hidden','true');f.style.cssText='position:fixed;width:1px;height:1px;right:-9999px;bottom:-9999px;border:0;opacity:0;pointer-events:none';document.body.appendChild(f);const width=paper==='58'?'54mm':'76mm',page=paper==='58'?'58mm':'80mm';const n=Math.max(1,Math.min(5,Number(copies)||1));const body=Array.from({length:n},(_,i)=>`<div class="print-copy">${html}</div>`).join('');const doc=`<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><style>@page{size:${page} auto;margin:2mm}body{margin:0;font-family:Arial,sans-serif;color:#000;direction:rtl}.print-copy{width:${width}}.print-copy:not(:last-child){page-break-after:always}.receipt{width:${width};font-size:12px;line-height:1.45}.receipt-logo{display:block;max-width:32mm;max-height:22mm;object-fit:contain;margin:2mm auto}.receipt h2{text-align:center;font-size:19px;margin:3px 0}.receipt h1{text-align:center}.receipt hr{border:0;border-top:1px dashed #000;margin:6px 0}.r-meta,.r-footer{text-align:center}.r-customer{line-height:1.7}.r-items>div,.r-totals>div{display:flex;justify-content:space-between;gap:8px;margin:5px 0}.r-items small{display:block}.grand-print{font-size:15px;font-weight:900;border-top:1px dashed #000;padding-top:6px}.shift-print{width:${width};font-size:11px}.shift-print h2,.shift-print h3{text-align:center;margin:5px 0}.shift-report-table{width:100%;border-collapse:collapse;font-size:10px}.shift-report-table th,.shift-report-table td{border-bottom:1px dashed #777;padding:4px 2px;text-align:right}.shift-report-table th:nth-child(n+2),.shift-report-table td:nth-child(n+2){text-align:left}.shift-report-table small{display:block;font-size:8px}</style></head><body>${body}</body></html>`;if(window.topBurgerDesktop?.print?.html){window.topBurgerDesktop.print.html(doc,{silent:false}).then(r=>{if(!r?.ok&&r?.error)toast('تعذر الطباعة: '+r.error)}).catch(e=>toast('تعذر الطباعة: '+e.message));f.remove();return}const d=f.contentDocument;d.open();d.write(doc);d.close();setTimeout(()=>{try{f.contentWindow.focus();f.contentWindow.print()}finally{setTimeout(()=>f.remove(),1200)}},150)}
