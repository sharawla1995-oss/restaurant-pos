...
modifiers:[],productModifiers:[],productVariants:[],deliveryZones:[],drivers:[],branchPrintSettings:[],employeeBranches:[],userPermissions:null,selectedCustomer:null,activeBranchId:null,homeBranchId:null,customerAddresses:[]};
...
const PERMISSION_GROUPS=[
  ...
  ['⚙️ النظام',['businessSettings','printingSettings']]
];
...
function canAccessPage(page){
  ...
  if(page==='settings')return isAdmin()||allowed.has('settings')||allowed.has('businessSettings')||allowed.has('printingSettings');
...
async function bootstrap(){
  const [emps,branches,cats,products,settingsRows,modifiers,productModifiers,productVariants,zones,drivers,printSettings]=await Promise.all([
  ...
  state.employee=emps[0];state.homeBranchId=Number(emps[0].branch_id||0);state.branches=branches||[];state.categories=cats||[];state.products=products||[];state.modifiers=modifiers||[];state.productModifiers=productModifiers||[];state.productVariants=productVariants||[];state.deliveryZones=zones||[];state.drivers=drivers||[];state.branchPrintSettings=printSettings||[];try{state.employeeBranches=await rest('employee_branches',`select=branch_id&employee_id=eq.${state.employee.id}`)}catch(e){state.employeeBranches=[]}try{state.userPermissions=await rest('employee_permissions',`select=permission_key,allowed&employee_id=eq.${state.employee.id}`)}catch(e){state.userPermissions=null}for(const r of (settingsRows||[])){if(r.key in state.settings)state.settings[r.key]=String(r.value)==='true';}
...
function receiptIdentityHTML(o,c){const b=branchById(o.branch_id);return `${c.show_logo&&state.business?.logo_url?`<img class="receipt-logo" src="${esc(state.business.logo_url)}" alt="">`:''}${c.show_business_name?`<h2>${esc(businessName())}</h2>`:''}<div class="r-meta">${c.receipt_header?`<b>${esc(c.receipt_header)}</b><br>`:''}<b>${esc(branchName(o.branch_id))}</b>${c.show_branch_phone&&b.phone?`<br>☎ ${esc(b.phone)}`:''}${c.show_branch_address&&b.address?`<br>${esc(b.address)}`:''}<br><b>${o.order_number||'#'+o.id}</b><br>${fmtDate(o.created_at||new Date().toISOString())}</div>`;}
function receiptHTML(o,items){const c=printCfg(o.branch_id);return `<div class="receipt">${receiptIdentityHTML(o,c)}<hr>${customerInfoHTML(o)}<div class="r-items">${items.map(i=>`<div><span>${esc(i.product_name)} × ${i.quantity}${i.notes?`<small>${esc(i.notes)}</small>`:''}</span><b>${money(i.total)}</b></div>`).join('')}</div><hr><div class="r-totals"><div><span>إجمالي الأصناف</span><b>${money(o.subtotal)}</b></div>${Number(o.discount||0)?`<div><span>خصم</span><b>-${money(o.discount)}</b></div>`:''}${Number(o.delivery_fee||0)?`<div><span>الدليفري</span><b>${money(o.delivery_fee)}</b></div>`:''}<div class="grand-print"><span>المطلوب</span><b>${money(o.total)}</b></div></div><hr><div class="r-footer">${orderTypeLabel(o.order_type)} • ${paymentLabel(o.payment_method)}<br>${esc(c.receipt_footer||state.business?.receipt_footer||'شكرًا لزيارتكم')}</div></div>`}
...
function prepReceiptHTML(o,items){const c=printCfg(o.branch_id);return `<div class="receipt"><div class="receipt-head"><h2>ريسيت التحضير</h2><h1>${o.order_number||'#'+o.id}</h1><p>${branchName(o.branch_id)} • ${fmtDate(o.created_at)}</p><p>${orderTypeLabel(o.order_type)}</p></div><hr>${customerInfoHTML(o)}<div class="r-items">${items.map(i=>`<div class="prep-item"><span><b>${i.quantity} × ${esc(i.product_name)}</b>${i.notes?`<small class="prep-note">📝 ${esc(i.notes)}</small>`:''}</span>${c.prep_show_prices?`<b>${money(i.total)}</b>`:''}</div>`).join('')}</div>${c.prep_show_prices?`<hr><div class="r-totals"><div><span>إجمالي الأصناف</span><b>${money(o.subtotal)}</b></div></div>`:''}<div class="prep-service-note">${c.prep_show_prices?'بدون رسوم خدمة/دليفري':''}</div></div>`}
...
async function renderSettings(){
 ...
 if(!canBusiness&&!canSystem&&!canPrinting){ $('#page').innerHTML='<div class="empty">ليس لديك صلاحية لفتح الإعدادات</div>'; return; }
 ...
 let systemPanel='',backupPanel='',printingPanel='';
 ...
 $('#page').innerHTML=businessPanel+printingPanel+systemPanel+backupPanel;
