...
const PERMISSION_DEFS=[
  ...
  ['websiteVisibility','🌐 التحكم في عرض منتجات الموقع'],
  ['branchProductAvailability','🏪 إدارة توفر الأصناف بالفروع']
];
...
async function renderProducts(){
 state.products=await rest('products','select=*&order=id'); state.modifiers=await rest('modifiers','select=*&order=id'); state.productModifiers=await rest('product_modifiers','select=*'); state.productVariants=await rest('product_variants','select=*&order=sort_order,id'); state.branchProducts=await rest('branch_products','select=*');
 ...
 <div class="toggle-row"><label><input id="pHasVariants" type="checkbox"> للصنف اختيارات / أحجام</label><label><input id="pExtras" type="checkbox" checked> يسمح بإضافات</label><label><input id="pRemove" type="checkbox" checked> يسمح بحذف مكونات</label><label><input id="pNotes" type="checkbox" checked> يسمح بملاحظة</label></div><div class="user-branches-box"><b>🏪 الفروع المتاح بها الصنف</b><div class="branch-checks">${state.branches.map(b=>`<label><input type="checkbox" class="new-product-branch" value="${b.id}" checked> ${esc(b.name)}</label>`).join('')}</div><small>تقدر تختار فرع واحد أو كل الفروع عند إضافة الصنف.</small></div>
 ...
 <div class="panel"><h2>الأصناف</h2><div class="table-wrap"><table><thead><tr><th>ترتيب</th><th>الصورة</th><th>الصنف</th><th>السعر</th><th>الاختيارات / الأحجام</th><th>الإضافات المتاحة</th><th>الفروع</th><th>الموقع</th><th>إجراء</th></tr></thead><tbody>${sortedActiveCategories().flatMap(c=>sortedActiveProducts(c.id)).map((p,idx,all)=>{const same=all.filter(x=>String(x.category_id)===String(p.category_id));const pos=same.findIndex(x=>String(x.id)===String(p.id));return `<tr><td class="sort-cell"><button class="secondary sort-arrow" data-product-up="${p.id}" ${pos===0?'disabled':''}>↑</button><button class="secondary sort-arrow" data-product-down="${p.id}" ${pos===same.length-1?'disabled':''}>↓</button></td><td>${p.image_url?`<img class="admin-product-img" src="${esc(p.image_url)}">`:'🍔'}</td><td>${p.name}</td><td>${money(p.price)}</td><td>${productVariantList(p).map(x=>`${esc(x.name)} ${money(x.price)}`).join('، ')||'-'}</td><td>${productModifierList(p).map(x=>x.name).join('، ')||'-'}</td><td><div class="branch-checks">${state.branches.map(b=>{const bp=(state.branchProducts||[]).find(x=>String(x.product_id)===String(p.id)&&String(x.branch_id)===String(b.id));const on=bp?bp.active!==false:true;return `<button class="${on?'primary':'secondary'}" data-branch-product="${p.id}" data-branch-id="${b.id}" ${hasFeaturePermission('branchProductAvailability')?'':'disabled'} title="${hasFeaturePermission('branchProductAvailability')?'تشغيل/إيقاف الصنف في '+esc(b.name):'ليس لديك صلاحية إدارة توفر الأصناف بالفروع'}">${on?'✅':'❌'} ${esc(b.name)}</button>`}).join(' ')}</div></td><td><button class="${p.website_visible!==false?'primary':'secondary'}" data-web-toggle="${p.id}" title="${hasFeaturePermission('websiteVisibility')?'إظهار/إخفاء الصنف على الموقع':'ليس لديك صلاحية التحكم في عرض الموقع'}" ${hasFeaturePermission('websiteVisibility')?'':'disabled'}>${p.website_visible!==false?'🌐 ظاهر':'🚫 مخفي'}</button></td><td><button class="secondary" data-image-product="${p.id}">🖼️ صورة</button> <button class="secondary" data-edit-product="${p.id}">✏️ تعديل</button> <button class="secondary" data-config="${p.id}">خيارات</button> <button class="danger" data-delete-product="${p.id}">🗑️ حذف</button></td></tr>`}).join('')}</tbody></table></div></div>`;
 ...
 $('#addProduct').onclick=async()=>{
  ...
  try{
   ...
   const selectedBranches=[...document.querySelectorAll('.new-product-branch:checked')].map(x=>Number(x.value));if(!selectedBranches.length)throw new Error('اختر فرعًا واحدًا على الأقل للصنف');for(const b of state.branches){try{await rest('branch_products','',{method:'POST',headers:{Prefer:'resolution=merge-duplicates'},body:JSON.stringify([{branch_id:b.id,product_id:pid,active:selectedBranches.includes(Number(b.id))}])})}catch(e){}}
