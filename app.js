...
async function renderReturns(){
 ...
 $('#findReturnOrder').onclick=async()=>{const q=await uiPrompt('اكتب رقم الفاتورة الداخلي المراد إرجاعها','',{title:'البحث عن فاتورة',type:'number',placeholder:'مثال: 1000'});if(q===null)return;const oo=await rest('orders',`select=*&branch_id=eq.${currentBranchId()}&invoice_number=eq.${Number(q)}&limit=1`);if(!oo.length)return toast('الفاتورة غير موجودة في الفرع الحالي');const it=await rest('order_items',`select=*&order_id=eq.${oo[0].id}&order=id`);openReturnForOrder(oo[0],it)};
