...
-- Corrective: restore authoritative Phase-7 Outer behavior and remove dummy stock identities.

-- Gate 3D-Corrective read-only preparation: derive deterministic real stock identities.
-- Base POS sale/return, expense and shifts are non-stock at this boundary.
...
begin
 ...
 if v_envelope is not null then v_context:=public.sharawla_point4_assert_context_envelope_v1(v_envelope,v_tx,v_branch);end if;
...
declare
 ...
 v_payload jsonb:=coalesce(p_event#>'{payload,rpc_payload}','{}'::jsonb);v_protocol integer:=coalesce(nullif(p_event->>'protocol_version','')::integer,0);
 v_device_id text:=nullif(trim(coalesce(p_event->>'device_id','')),'');v_sequence bigint:=coalesce(nullif(p_event->>'device_sequence','')::bigint,0);
 v_branch bigint:=coalesce(nullif(p_event->>'branch_id','')::bigint,0);v_employee bigint:=coalesce(nullif(p_event->>'employee_id','')::bigint,0);v_current_employee bigint;
 v_dep_tx text:=nullif(trim(coalesce(p_event->>'depends_on_tx_id','')),'');v_dep_map_tx text:=nullif(trim(coalesce(p_event#>>'{dependency_mapping,client_tx_id}','')),'');v_dep_server_id text:=nullif(trim(coalesce(p_event#>>'{dependency_mapping,server_id}','')),'');
 v_receipt public.offline_v2_server_receipts%rowtype;v_result jsonb;v_entity_id text;v_event_id text;
 v_prepared jsonb;v_envelope jsonb;v_stock_identities jsonb;v_context jsonb;v_identity record;v_forward jsonb;
begin
 -- Authoritative Phase-7 behavior from 28c3320... remains owned by Outer.
 if v_operation in('customer_merge','order_status') then
  if auth.uid() is null then raise exception using errcode='42501',message='غير مصرح';end if;
  if v_protocol<>2 or v_tx is null or v_digest is null or v_rpc is null or v_device_id is null or v_sequence<=0 or v_branch<=0 or v_employee<=0 then raise exception using errcode='22023',message='Offline V2 Phase 7 event contract غير مكتمل';end if;
  v_current_employee:=public.current_employee_id();if v_current_employee is null or v_current_employee is distinct from v_employee then raise exception using errcode='42501',message='بيانات الموظف غير مطابقة لجلسة المزامنة';end if;
  if (v_operation='customer_merge' and v_rpc<>'offline_v2_merge_customer_v1') or (v_operation='order_status' and v_rpc<>'offline_v2_update_order_status_v1') then raise exception using errcode='22023',message='Offline V2 Phase 7 operation/RPC binding غير مدعومة';end if;
  if nullif(trim(coalesce(v_payload->>'p_client_tx_id','')),'') is distinct from v_tx then raise exception using errcode='22023',message='Offline V2 Phase 7 client_tx_id mismatch';end if;
  if v_operation='order_status' and v_dep_tx is not null then
   if v_dep_server_id is null or v_dep_map_tx is distinct from v_dep_tx then raise exception using errcode='22023',message='Offline V2 order status dependency mapping غير مكتملة';end if;
  ...
  if v_operation='customer_merge' then v_result:=public.offline_v2_merge_customer_v1(v_payload->>'p_name',v_payload->>'p_phone',v_payload->>'p_area',v_payload->>'p_address',v_payload->>'p_notes',v_tx);v_entity_id:=nullif(v_result->>'customer_id','');
  else
   if coalesce(nullif(v_payload->>'p_branch_id','')::bigint,0) is distinct from v_branch then raise exception using errcode='22023',message='Offline V2 order status branch mismatch';end if;
   v_result:=public.offline_v2_update_order_status_v1((v_payload->>'p_order_id')::bigint,v_branch,v_payload->>'p_status',nullif(v_payload->>'p_driver_id','')::bigint,v_payload->>'p_cancelled_reason',v_tx);v_entity_id:=nullif(v_result->>'order_id','');
  end if;
 ...
 if v_prepared->>'classification'='FULL_REPLAY' then return public.sharawla_offline_v2_apply_event_core_v1(p_event);end if;
 if v_prepared->>'classification'<>'EXECUTION_REQUIRED' then raise exception 'Offline preparation classification غير صالحة';end if;
 v_envelope:=v_prepared->'context_envelope';v_stock_identities:=coalesce(v_prepared->'stock_identities','[]'::jsonb);
 if v_envelope is not null then v_context:=public.sharawla_point4_assert_context_envelope_v1(v_envelope,v_prepared->>'client_tx_id',(v_prepared->>'branch_id')::bigint);end if;
 for v_identity in select (x->>'location_id')::bigint location_id,x->>'item_kind' item_kind,(x->>'item_id')::bigint item_id from jsonb_array_elements(v_stock_identities) x
 loop
  if v_identity.location_id is distinct from v_branch or v_identity.item_kind not in('product','variant','ingredient') or coalesce(v_identity.item_id,0)<=0 then raise exception 'Point4 Offline stock identity غير صالحة';end if;
  perform public.inventory_stock_assert_legacy_write_allowed_v2(v_identity.location_id,v_identity.item_kind,v_identity.item_id);
 end loop;
 v_forward:=jsonb_set(jsonb_set(p_event,'{point4_context_envelope}',coalesce(v_envelope,'null'::jsonb),true),'{point4_stock_identities}',v_stock_identities,true);
 return public.sharawla_offline_v2_apply_event_core_v1(v_forward);
end;$function$;

-- Preserve Phase-7 hardening: Core/helpers remain internal; authenticated calls enter through Outer only.
