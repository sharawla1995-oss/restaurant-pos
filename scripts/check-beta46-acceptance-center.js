...
for(const t of ['permissions.all-profile-role-contracts','permissions.current-session-boundary','capabilities.enabled-feature-test-coverage','recovery.guard-health','auth.offline-cache-boundary','recovery.backup-lastgood-restore-copy','recovery.temp-restore-cycle','recovery.corruption-copy-detection','recovery.clock-drift-device-sequence','permissions.backend-role-impersonation','recovery.live-db-restore','system_clock_changed','restored_into_live'])need(recoveryPermissions,t);
