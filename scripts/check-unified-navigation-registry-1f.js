...
// Regression guard: navActive must iterate the full nav collection, never a single $() element.
if(!app.includes("function navActive(p){$('#nav button').forEach(")) errors.push('navActive must use $(#nav button) collection before forEach');
if(app.includes("function navActive(p){$('#nav button').forEach(")) errors.push('navActive single-element $() regression detected');
