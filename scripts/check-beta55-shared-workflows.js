...
need(foundation,["location_type text not null default 'branch'","check (location_type in ('branch','central_warehouse'))",'inventory_supply_routes','inventory_supply_catalog','inventory_supply_requests','inventory_supply_request_items','inventory_supply_request_events',"public.has_action_permission_v2('inventory.supply.configure')","public.has_action_permission_v2('inventory.supply.request.create')","public.has_action_permission_v2('inventory.supply.request.approve')",'public.has_branch_access','inventory_supply_request_create_v1','inventory_supply_request_submit_v1','inventory_supply_request_decide_v1'],'Beta55 supply foundation');
...
const acc=read('owner-acceptance-beta55-v55.js'),registry55=read('owner-acceptance-registry-v55.js'),lazy=read('owner-acceptance-lazy-loader-v47.js'),accUi=read('owner-acceptance-ui-v47.js');
...
need(lazy,['owner-acceptance-registry-v55.js?v=10.5.4-beta.55','owner-acceptance-beta55-v55.js?v=10.5.4-beta.55','sharawla-beta55-acceptance-ready'],'Beta55 acceptance lazy loader');
...
const order=['beta55-ui-workflow-fixes.js','beta55-ui-hardening.js','beta55-central-warehouse-ui.js','beta55-central-warehouse-v2.js','owner-acceptance-lazy-loader-v47.js'].map(x=>loader.indexOf(x));
if(order.some(x=>x<0)||!(order[0]<order[1]&&order[1]<order[2]&&order[2]<order[3]&&order[3]<order[4]))throw new Error('Beta55 loader order invalid');
need(loader,['beta55CentralWarehouseV2:true'],'Beta55 loader marker');

for(const src of [ui,hard,supply,supplyV2,foundation,fulfill,hardened,visibility,branchRead,allocation]){
...
console.log('Beta55 shared workflow / Warehouse V2 / v55 Acceptance static gate OK.');
