...
for(const t of ['sharawla_offline_v2_apply_event','sharawla_offline_v2_transport_info','offline-v2:sync-now','offline-v2:manual-retry','offline-v2:transport-attest','OFFLINE_V2_LEGACY_AUTHORITY_ACTIVE','employee_id=?','device_id=?','business_id=?'])need(transportSource,t);
for(const t of ['create_food_pos_order_atomic_v1','create_food_retail_pos_order_atomic_v1','create_retail_variant_pos_order_atomic_v1','create_food_order_return_idempotent_v1','create_retail_variant_order_return_idempotent_v1','rpc_name','rpc_payload','depends_on_tx_id','transportAttest','manualRetry'])need(runtimeSource,t);
for(const t of ['syncNow:x=>ipcRenderer.invoke','manualRetry:x=>ipcRenderer.invoke','transportAttest:x=>ipcRenderer.invoke','data-offline-v2-phase5'])need(preloadSource,t);
need(mainSource,"require('./beta45-offline-v2-transport.js').installOfflineV2Transport(offlineV2Store)");
for(const t of ['offline_v2_server_receipts','enable row level security','security definer','sharawla_offline_v2_apply_event','sharawla_offline_v2_transport_info','authenticated','client_tx_id','payload_digest','server_event_id','idempotent_replay','operation/RPC binding','RPC client_tx_id mismatch'])need(sqlSource,t);
for(const rpc of [
...
(async()=>{
  // Auth/session failures block instead of retry-hammering or deleting work.
  ...
  // Permanent transport contract errors enter DLQ immediately.
  ...
  // Protocol corruption gets bounded retries, then DLQ on threshold.
  ...
  // Stock validation is a durable conflict and independent work still proceeds.
  ...
  // A synced parent without a mapping does not leak a local ID to the server.
  ...
  console.log('Beta45 Offline V2 Phase 5 transport / conflict / retry / DLQ gate PASS');
