...
const offline=[read('beta45-offline-v2-transport-runtime.js'),read('beta45-offline-v2-native-store.js'),read('beta45-offline-v2-sync.js')].join('\n');
...
assert(/source_document_id text not null/.test(foundation)&&/p_source_document_id text default null/.test(writer));assert(!/point4_identity_/i.test(foundation+writer));assert(/payloadDigest|payload_digest/.test(offline));assert(!/point4_identity_|sharawla\.point4\.identity/i.test(offline));assert(!/\bbusiness_id\s+bigint\b/i.test(journal.replace(/^\s*--.*$/gm,'')));
