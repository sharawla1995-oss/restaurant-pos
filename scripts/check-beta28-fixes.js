...
if(pkg.version!=='10.5.4-beta.28')throw new Error(`Expected 10.5.4-beta.28, got ${pkg.version}`);
...
for(const token of ['اسم المستخدم<input id="email" type="text"','beta28-runtime-fixes.js?v=10.5.4-beta.28'])if(!index.includes(token))throw new Error(`Beta28 index invariant missing: ${token}`);
for(const token of [
  "action:'login'",
...
if(!sw.includes(`./beta28-runtime-fixes.js?v=${pkg.version}`)||!sw.includes("url.pathname.endsWith('/beta28-runtime-fixes.js')"))throw new Error('Service worker missing Beta28 runtime fixes');
...
console.log('Beta28 Owner/User/Home gate OK');
