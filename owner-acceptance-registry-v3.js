(function(global){
'use strict';
const VERSION='10.5.4-beta.46-acceptance-registry-v3';
...
async function executeTest(t,ctx){
 const started=now(),previous=lastResult(t.id);
...
async function start(opts={}){
 ...
 current={schema:3,run_id:runId(),status:'RUNNING',level,mode,profile,started_at:now(),environment:await environment(),baseline_before:await baseline(),results:[],selected:selected.map(t=>t.id)};persistActive();
...
current=json(ACTIVE_KEY,null);if(current?.status!=='RUNNING')current=null;
