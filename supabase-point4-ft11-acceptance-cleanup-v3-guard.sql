CREATE OR REPLACE FUNCTION public.sharawla_acceptance_cleanup_v3(p_run_id text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
declare
 v_uid uuid:=auth.uid();
 v_orders bigint[]:=array[]::bigint[]; v_returns bigint[]:=array[]::bigint[];
 v_ship bigint[]:=array[]::bigint[]; v_settle bigint[]:=array[]::bigint[];
 ...
 v_po bigint[]:=array[]::bigint[]; v_grn bigint[]:=array[]::bigint[]; v_sr bigint[]:=array[]::bigint[];
 v_batches bigint[]:=array[]::bigint[]; v_rx bigint[]:=array[]::bigint[]; v_comp bigint[]:=array[]::bigint[];
...
begin
 ...
 -- Point 4: freeze V3 acceptance retail ownership after both zero-state gates.
 ...
 select coalesce(array_agg(id),array[]::bigint[]) into v_grn from public.retail_goods_receipts where client_tx_id like p_run_id||'%';
 select coalesce(array_agg(id),array[]::bigint[]) into v_sr from public.retail_supplier_returns where client_tx_id like p_run_id||'%';
 ...
 select coalesce(array_agg(id),array[]::bigint[]) into v_comp from public.pharmacy_insurance_companies where lower(code)=lower(p_run_id||'-INS');
 ...
 delete from public.membership_plans_v1 where lower(code)=lower(p_run_id||'-PLAN');
 ...
 delete from public.service_packages_v1 where lower(code)=lower(p_run_id||'-PKG');
 delete from public.service_catalog where lower(code)=lower(p_run_id||'-SVC');
 ...
 delete from public.pharmacy_insurance_plans where company_id=any(v_comp) or lower(code)=lower(p_run_id||'-PLAN');
 delete from public.pharmacy_insurance_companies where id=any(v_comp);

 delete from public.retail_supplier_return_items where supplier_return_id=any(v_sr);
 delete from public.retail_supplier_returns where id=any(v_sr);
 delete from public.retail_goods_receipt_items where goods_receipt_id=any(v_grn);
 delete from public.retail_purchase_workflow_events where (entity_type='goods_receipt' and entity_id=any(v_grn)) or (entity_type='purchase_order' and entity_id=any(v_po));
 delete from public.retail_goods_receipts where id=any(v_grn);
 ...
 v_after:=public.sharawla_acceptance_scan_v3(p_run_id);
