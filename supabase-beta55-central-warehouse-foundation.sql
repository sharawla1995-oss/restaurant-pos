...
-- -----------------------------------------------------------------------------
create or replace view public.inventory_supply_catalog_live_v1 as
select
  ...
  case
    when c.item_type='product' then coalesce(srcp.quantity,0)
    else coalesce(srci.quantity,0)
  end::numeric(14,3) as source_quantity,
  case
    when c.item_type='product' then coalesce(dstp.quantity,0)
    else coalesce(dsti.quantity,0)
  end::numeric(14,3) as destination_quantity,
  c.sort_order,
  c.notes
...
begin
 ...
 insert into public.audit_logs(employee_id,branch_id,action,entity_type,entity_id,details)
 values(v_emp,v_q.source_location_id,case when coalesce(p_approve,false) then 'inventory.supply.request.approve' else 'inventory.supply.request.reject' end,'inventory_supply_request',v_q.id,jsonb_build_object('destination_branch_id',v_q.destination_branch_id));
 return v_q.id;
end$$;
...
begin
 if auth.uid() is null then raise exception 'غير مصرح';end if;
 if not public.has_action_permission_v2('inventory.supply.request.create') then raise exception 'ليس لديك صلاحية إلغاء طلب التوريد';end if;
 select * into v_q from public.inventory_supply_requests where id=p_request_id for update;
 if not found then raise exception 'طلب التوريد غير موجود';end if;
 if not public.has_branch_access(v_q.destination_branch_id) then raise exception 'ليس لديك صلاحية الفرع الطالب';end if;
 if v_q.status='cancelled' then return v_q.id;end if;
 if v_q.status not in ('draft','submitted') then raise exception 'لا يمكن إلغاء الطلب بعد الاعتماد أو التجهيز';end if;
 v_emp:=public.current_employee_id();
 ...
 insert into public.inventory_supply_request_events(request_id,from_status,to_status,note,employee_id) values(v_q.id,v_q.status,'cancelled',nullif(trim(coalesce(p_note,'')),''),v_emp);
 insert into public.audit_logs(employee_id,branch_id,action,entity_type,entity_id,details) values(v_emp,v_q.destination_branch_id,'inventory.supply.request.cancel','inventory_supply_request',v_q.id,'{}'::jsonb);
 return v_q.id;
end$$;
