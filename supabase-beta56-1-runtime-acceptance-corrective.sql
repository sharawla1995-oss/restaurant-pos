...
begin
 ...
 if v_envelope is not null and jsonb_typeof(v_envelope)<>'null' then v_context:=public.sharawla_point4_assert_context_envelope_v1(v_envelope,v_prepared->>'client_tx_id',(v_prepared->>'branch_id')::bigint);end if;
 ...
 v_forward:=jsonb_set(p_event,'{point4_stock_identities}',v_stock_identities,true);
 if v_envelope is not null and jsonb_typeof(v_envelope)<>'null' then
  v_forward:=jsonb_set(v_forward,'{point4_context_envelope}',v_envelope,true);
 else
  v_forward:=v_forward-'point4_context_envelope';
 end if;
