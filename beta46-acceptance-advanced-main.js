...
function crashRestart(input={}){const marker=writeMarker(input);const delay=Math.max(150,Math.min(2500,Number(input.delay_ms||450)));setTimeout(()=>{try{app.relaunch()}finally{process.exit(86)}},delay);return {ok:true,restarting:true,abrupt:true,delay_ms:delay,marker,marker_path:markerPath()}}
