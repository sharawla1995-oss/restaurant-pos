-- Point4 Pre-Cutover 46 Guard Installation — Batch 4
-- Contract #21 only: retail_create_website_order_identity_v1(jsonb).
-- Source-only. No deployment / activation / cutover.
-- Frozen affected set = validated deterministic v_legacy_items aggregate.
...
begin
  ...
  -- Point4 Batch 4: guard exactly the validated frozen aggregate before the
  -- first durable lifecycle write. Execution below continues from v_legacy_items.
  for v_guard_product_id in
    select distinct x.product_id
      from jsonb_to_recordset(v_legacy_items) as x(product_id bigint)
